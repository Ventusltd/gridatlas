...
export function validateReleaseManifest(manifest, { sealed = true } = {}) {
  ...
  invariant(manifest.release_id === "202608291430-atlas-v9", "application release identity mismatch");
  invariant(manifest.generation === "202608291430", "application generation mismatch");
  ...
  invariant(manifest.parent_release?.publication_commit === "1898184ccbf52ca836cf1482362fc5933baf3e8d", "last-green parent drift");
  invariant(manifest.repd?.generation === "202608290716" && manifest.repd.rows === 11069, "REPD preservation contract mismatch");
  invariant(manifest.repd?.selectable_rows === 11033 && manifest.repd.excluded_false_origin_rows === 36, "safe REPD closure mismatch");
  ...
  if (sealed) {
    for (const [name, value] of Object.entries({
      ...
      routing_publication_commit: manifest.repd_routing.publication_commit,
    ...
    invariant(SHA256.test(manifest.repd_routing.release_sha256), "routing release digest is invalid");
