 ...
 *
 * Generation 202609011751 (UTC), composition v9.57. Slot: replace-script for
...
(() => {
  ...
  function provisionalDeclaredConnection(repdRef) {
    ...
    return {
      ...
      works: SUBSTATION_WORKS[declared.substation.toLowerCase()] || null,
  ...
  function resolveDeclaredConnection(repdRef, origin, subs) {
    const declared = DECLARED_CONNECTIONS[String(repdRef || '')];
    if (!declared) return null;
    if (declared.poc_kind === 'circuit') return provisionalDeclaredConnection(repdRef);
  ...
  function declaredBlockHtml(toSubstations) {
    ...
    /* What the system operator publishes about the substation this card
       names. The sandbox asks; the substation-intelligence cartridge
       answers from Ventusltd/data-grid-gb, which is ETYS restated. Absent
       is absent: no sentence at all rather than an empty one. */
    const networkName = currentDeclared?.kind !== 'circuit'
      ? (currentDeclared?.poc || currentNearest400?.name) : currentNearest400?.name;
    const published = (() => {
      try { return window.__GRIDATLAS_NETWORK__?.summarise?.(networkName) || null; }
      catch (_) { return null; }
    })();
    if (published) {
      out += `<div class="neon-hd">${escapeHtml(networkName)}`
        ...
        + `<p class="neon-caveat">${escapeHtml(published.attribution)}. `
