/**
 * sld-sandbox-v9-8, generation 202609040046 (UTC).
 *
 ...
 * hashed in manifests/202609040046-sld-sandbox-v9-8-parts.json.
 *
...
(() => {
  ...
  /* The receiver decision is pure and exported because Pipeline News owns
     ...
     plan; the corpus proof can therefore pass all 8,756 published points
      ...
      try {
        ...
        if (receiverPlan.route === 'MEASURE_LINK_FIRST' && repdRef) {
          ...
          identityVerification = waitForResolvedIdentity({ announce: false })
            .then((resolved) => ({
              resolved,
              terminal: window.__GRIDATLAS_PLACE_SEARCH__?.deep_link?.status || 'UNKNOWN'
            }))
        ...
        try {
          ...
          link.camera_from_link = { longitude: lon, latitude: lat,
            zoom: arrivalZoom, reason: identityVerification
              ...
              : (repdRef ? 'resolved repd_ref coordinates' : 'no repd_ref') };
        } catch (error) {
        ...
        let currentArrival = Object.freeze({ lon, lat, name, tech, stated, repdRef });
        const enableBoth = () => {
        ...
        async function runArrivalSelection(arrival, waitForOwnerCard = false) {
          ...
          await selectAt([arrival.lon, arrival.lat], arrival.name,
            arrival.tech, false,
        ...
        if (identityVerification) {
          ...
          identityVerification.then(async ({ resolved, terminal, error }) => {
            if (epoch !== arrivalEpoch) return;
            if (!resolved) {
              link.origin_source = 'link-supplied-register-' + String(terminal).toLowerCase();
            ...
            try {
              ...
              await runArrivalSelection(verifiedArrival);
              enableBoth();
