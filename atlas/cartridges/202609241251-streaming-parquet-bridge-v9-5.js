(() => {
  ...
  async function resolvePartition(pathname) {
    ...
    // These three controls draw stations. The root partition contains track lines.
    // The station partition is selected explicitly after geometry/filter/browser validation;
    // provenance and the former quarantine are recorded in the composition manifest.
    const alias = stem === 'uk_metros_trams' ? 'uk_metros_trams_local_unwired' : stem;
