/**
 * sld-sandbox-v9-8, generation 202609040021 (UTC).
 *
 ...
 * hashed in manifests/202609040021-sld-sandbox-v9-8-parts.json.
 *
...
(() => {
  ...
  (function dashCollapse() {
    ...
    /* Closed on a FIRST arrival only - measured, the panel held 31.6% of a
       393x852 screen and the map 29.3%. A choice once made still wins. */
    let collapsed = true;
    try {
  ...
  async function waitForResolvedIdentity() {
    ...
    async function runDeepLink() {
      try {
        ...
        /* THE REGISTER OWNS THE COORDINATES. THE URL ONLY RESTATES THEM.
           This guard read (!coordsUsable() || !isProjectTech(tech)), so the
           register was consulted only when the LINK was malformed - leaving
           the well-formed-but-wrong link the one case never defended, which
           printed a real project's address and planning status over a
           measurement 30 km away with nothing on screen contradicting it.
           The rule is the comment above; the code honoured it for technology,
           name and capacity and omitted the pair that decides WHERE.
           Measured: sessions/202609031759-identity-leak/00-FINDING.md. */
        if (q.get('repd_ref')) {
          ...
          if (resolved) {
            ...
            if (Number.isFinite(rLon) && Number.isFinite(rLat)
              ...
              /* Published before it is overwritten: the page held both points
                 all along and never subtracted them. */
              if (coordsUsable()) {
                link.origin_discrepancy_km =
                  Math.round(distanceKm(lon, lat, rLon, rLat) * 1000) / 1000;
              }
              lon = rLon;
              lat = rLat;
              link.origin_source = 'register';
            } else {
              // Resolved, carrying no usable point. The URL is then all there
              // is, and is recorded as such rather than passed off.
              link.origin_source = 'link';
            }
          ...
          } else if (coordsUsable()) {
            /* Unresolved: no register identity is rendered, so there is
               nothing for a measurement to be falsely paired with. The link's
               own point is the honest arrival and keeps it alive. */
            link.origin_source = 'link';
          } else {
        ...
        /* A link with coordinates and no repd_ref moved nothing. The shell
           returns at its repd_ref pattern test before reaching any flyTo,
           and the search lane returns `status: 'ABSENT'` at the same test,
           so both lanes that own the camera stood down; honourRequestedZoom
           then eased the ZOOM with no CENTRE set, zooming in on the default
           view. So the arrival flies here, ONLY when there is no repd_ref -
           with an identity the shell's flyTo runs and a second would fight
           it. Zoom is the link's where usable and 12 otherwise, which is
           what the shell hard-codes and Pipeline News sends. `essential`,
