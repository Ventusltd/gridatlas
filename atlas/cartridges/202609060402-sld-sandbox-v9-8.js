/**
 * sld-sandbox-v9-8, generation 202609060402 (UTC).
 *
 ...
 * hashed in manifests/202609060402-sld-sandbox-v9-8-parts.json.
 *
 ...
 *   part                   atlas/parts/202609060402-sld-sandbox-transmission-line.js
 */
...
(() => {
  ...
  function drawLinks(map, origin, name, tech, links, direction, statedMw, layerLoaded = true) {
    ...
    // The 400 kV measurement is independent of the bounded nearby search.
    // Draw its measured endpoint even when no qualifying station is within 40 km.
    // Keep it separate from public declared connections and nearby rank ordering.
    if (direction === 'to-substation' && currentNearest400?.at &&
        !links.some(item => item.at[0] === currentNearest400.at[0] && item.at[1] === currentNearest400.at[1])) {
      const measured = currentNearest400;
      lines.push({type:'Feature',properties:{colour:'#ffd166',strength:0.75,km:measured.km,role:'indicative-nearest-transmission'},geometry:{type:'LineString',coordinates:[origin,measured.at]}});
      nodes.push({type:'Feature',properties:{colour:'#ffd166',role:'indicative-nearest-transmission',label:`Indicative 400 kV - ${measured.km.toFixed(2)} km straight`},geometry:{type:'Point',coordinates:measured.at}});
    }
  ...
  function installMobileTray() {
    ...
    /* GRID AND SUBS ON DESKTOP TOO.
       ---------------------------------------------------------------------
       "in mobile view the Grid and subs buttons are nice show them on desktop
       TOO" -- the architect, 2026-09-05. This used to return here, so on a
       fine pointer the two chips were never CREATED at all; a later attempt to
       fix this by changing where the menu bar hoists tray buttons did nothing,
       because there was nothing to hoist.

       What stays phone-only is the COLLAPSE: folding the six tool buttons
       behind one chip exists because on a 393px portrait map they cover a
       third of the screen, which is not true on a desktop and would be a
       regression there. So `collapse` gates the tray behaviour, and the two
       chips are built either way. */
    const collapse = trayTarget();
    link.mobile_tray = { installed: true, collapsed: collapse,
      reason: collapse ? 'coarse pointer or narrow window' : 'chips only, fine pointer' };
    const stack = document.querySelector('.map-controls');
    ...
    if (collapse) stack.classList.add('gm-tools-collapsed');
    ...
    /* Only GRID and SUBS cross to the desktop. The tools chip is the collapse
       itself, and Scope and Clear are already one visible click away in the
       menu there -- adding them would be redoing the desktop toolbar, which is
       not what was asked for. */
    if (collapse) tray.appendChild(tools);
    tray.appendChild(grid);
    tray.appendChild(subs);
    if (collapse) tray.appendChild(scopeChip);
    if (collapse) tray.appendChild(clearChip);
