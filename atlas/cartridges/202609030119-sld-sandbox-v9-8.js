/**
 * sld-sandbox-v9-8, generation 202609030119 (UTC).
 *
 ...
 * hashed in manifests/202609030119-sld-sandbox-v9-8-parts.json.
 *
...
(() => {
  ...
  function declaredBlockHtml(toSubstations) {
    ...
    const connectionKv = currentDeclared?.kind !== 'circuit'
      ...
      : null;
    /* The connection-points product counts LANDINGS, not machines: a
       transformer's two windings are both at the site, so it is published
       at each of them. Cowley's five machines were reported as ten, and
       484 of the 525 sites holding a transformer were overstated 1.90x.
       Only the node/branch model holds the pairs needed to deduplicate,
       so where it has been indexed the unit counts are handed to the
       summariser; where it has not, the summariser names the figure for
       what it is rather than calling landings machines. */
    ...
    if (published) {
      ...
      out += `<div class="neon-hd">${escapeHtml(networkName)}`
  ...
  const TOPOLOGY_PRODUCT = 'https://raw.githubusercontent.com/Ventusltd/data-grid-gb/'
    + 'main/derived/gb-transmission-network.v1.json';
  ...
  (function dashCollapse() {
    ...
    /* HIDE LAYERS collapsed `.dashboard`, and `.dashboard` is the whole
       application.
       ------------------------------------------------------------------
       In the shipped shell `.dashboard` opens at index.html:22 and holds
       BOTH `.map-container` (line 36, which contains #map) and
       `.scada-wrapper` (line 112, which holds the layer keys, the status
       legend and the disclaimer). Collapsing it to max-height:0 with
       overflow:hidden therefore took the WebGL canvas down with the
       checkboxes: on a phone, where the control is the only way past an
       816 px panel, the button blanked the app and the only recovery was
       a reload with the choice remembered in localStorage - so the reload
       blanked it again.

       The control targets `.scada-wrapper` instead, which is the layer
       panel and nothing else. If the shell ever stops publishing that
       wrapper the control is NOT installed against a fallback: the last
       fallback is what blanked the map, and no control is better than one
       that can hide the map.

       It is also hidden while a fullscreen element is present. The
       fullscreen keeper below MOVES the layer panel into the fullscreen
       element, so a fixed-position button outside it either does nothing
       or acts on a node that is no longer where the reader is looking. */
  ...
  function topologyInnerHtml(queries) {
    ...
    for (const q of queries) {
      ...
      if (!facts.by_voltage.length) {
        ...
        /* The site-wide totals are PHYSICAL UNITS. The per-voltage lines
           below them are landings, and they will not add up to these:
           a transformer whose windings are both here appears under each
           voltage, which is what a reader at a busbar wants and is the
           reason the site-wide line cannot be their sum. */
  ...
  async function waitForResolvedIdentity() {
    ...
    async function runDeepLink() {
      try {
        ...
        /* A link that carries coordinates but no repd_ref moved nothing.
           ------------------------------------------------------------------
           Nobody flew the camera on that link. The shell's own arrival
           handler returns at `if (!/^[A-Za-z0-9-]{1,40}$/.test(repdRef))
           return;` before it reaches any flyTo, and the search lane returns
           `status: 'ABSENT'` at the same test - so the two lanes that own the
           camera both stood down and the map stayed wherever it had opened.
           honourRequestedZoom then eased the ZOOM without ever setting the
           CENTRE, which zooms in on the default view: worse than not moving,
           because it looks deliberate.

           So the arrival flies here, and ONLY when there is no repd_ref. With
           an identity present the shell's flyTo runs and a second one from
           this cartridge would fight it - that path is untouched.

           The centre comes from the link. The zoom is the link's own where it
           passed the usability test above, and otherwise 12, which is what the
           shell has always used and what Pipeline News has always sent, so an
           existing link lands exactly where it landed before. `essential`
           because a reader who has asked the operating system for reduced
           motion still needs the camera to arrive. */
        if (q.get('repd_ref') === null) {
          try {
            ...
            link.camera_from_link = { longitude: lon, latitude: lat,
              zoom: arrivalZoom, reason: 'no repd_ref, so no other lane flies' };
          } catch (error) {
        ...
        async function runArrivalSelection() {
          ...
          try { if (capturedMap) setPin(capturedMap, [lon, lat], name, tech); }
