/**
 * sld-sandbox-v9-8, generation 202609030128 (UTC).
 *
 ...
 * hashed in manifests/202609030128-sld-sandbox-v9-8-parts.json.
 *
...
(() => {
  ...
  function declaredBlockHtml(toSubstations) {
    ...
    const connectionKv = currentDeclared?.kind !== 'circuit'
      ...
      : null;
    /* connection-points counts LANDINGS, not machines: a transformer's
       windings are both at the site, so it is published at each. Cowley's
       five read as ten; 484 of 525 sites were 1.90x over. Only the
       node/branch model holds the pairs, so the units go over when it has
       been indexed and the summariser names the figure otherwise. */
  ...
  (function dashCollapse() {
    ...
    /* HIDE LAYERS collapsed `.dashboard`, which is the whole app.
       ------------------------------------------------------------------
       In the shipped shell `.dashboard` opens at index.html:22 and holds
       BOTH `.map-container` (line 36, containing #map) and
       `.scada-wrapper` (line 112, the layer keys and legend). Collapsing
       it to max-height:0 took the WebGL canvas down with the checkboxes,
       and the choice is remembered, so the reload a reader reaches for
       blanked the page again.

       It targets `.scada-wrapper` now. There is NO fallback: the last
       fallback is what blanked the map, and no control is better than one
       that can hide it. It also hides itself while a fullscreen element is
       present, because the keeper below MOVES the panel into that element
       and a fixed button outside it acts on a node the reader has left. */
  ...
  function topologyInnerHtml(queries) {
    ...
    for (const q of queries) {
      ...
      if (!facts.by_voltage.length) {
        ...
        /* Site totals are UNITS; the per-voltage lines below are
           landings and will not add up to them, because a transformer
           with both windings here appears under each of its voltages. */
  ...
  async function waitForResolvedIdentity() {
    ...
    async function runDeepLink() {
      try {
        ...
        /* A link with coordinates and no repd_ref moved nothing.
           ------------------------------------------------------------------
           The shell returns at its repd_ref pattern test before reaching any
           flyTo, and the search lane returns `status: 'ABSENT'` at the same
           test, so both lanes that own the camera stood down.
           honourRequestedZoom then eased the ZOOM with no CENTRE ever set,
           zooming in on the default view: worse than not moving, because it
           looks deliberate.

           So the arrival flies here, ONLY when there is no repd_ref - with an
           identity the shell's own flyTo runs and a second would fight it.
           Zoom is the link's where usable and 12 otherwise, which is what the
           shell hard-codes and Pipeline News sends, so an existing link lands
           where it always did. `essential`, because a reader who asked for
           reduced motion still needs the camera to arrive. */
        ...
        /* An unrecognised technology used to abandon the whole arrival.
           ------------------------------------------------------------------
           `return` here cost the card, the ring, the nearest-substation
           measurement, the declared connection and the substation layer -
           all arithmetic over two coordinates and a register row, none of
           which needs to know what a project generates. Only the one
           technology layer does, so that is all an unknown id costs now.
           `PROJECT_TECHS` accepts 11,065 of the 11,069 ids the register
           writes; the four it missed are added above. What the guard really
           catches now is a link that omits or garbles the parameter, or
           carries an id from a newer register - and for all three the right
           answer is the map, not a blank.

           Recorded on its own surface, not in `link.failures`, which since
           202609011434 means the arrival lost something. This one did not. */
        const technologyKnown = isProjectTech(tech);
        ...
        const enableBoth = () => {
          ...
          if (technologyKnown) enableTechnologyLayer(tech);
        };
        const arrive = async () => {
          clearStatus();
