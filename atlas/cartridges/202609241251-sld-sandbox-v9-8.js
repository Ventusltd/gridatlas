/**
 * CARRIED FORWARD AND EDITED, NOT ASSEMBLED. The notice below was true at generation 202609050354. Every generation
 * since has been cut by copying the previous cartridge and editing the copy; the parts it names were not touched again
 * and no longer reproduce this file (they build 363 KB; this is over 420 KB). Do NOT rebuild this cartridge from those
 * parts: that would silently discard everything added since 5 September. Said here at 202609200022 because a header
 * that misdescribes how a file is made is the kind of false record this estate keeps finding.
 *
...
(() => {
  ...
  let currentNeso = null;
  let declaredFrameGuard = null;   // detaches the frame guard; see drawLinks
  ...
  /* THE CONNECTION AUTHORITY'S OWN ANSWER.
     ---------------------------------------------------------------------
     An offshore wind farm has to make landfall. Its export cable comes
     ashore and connects at an onshore substation, and this engine had no
     idea which one - so it answered the only question it could, "what is
     the nearest mapped substation to the array", and for an array sitting
     60 km out at sea that returns another company's offshore platform.
     Berwick Bank was told its nearest substation is the Neart na Gaoithe
     platform. Hornsea 3 was told Sheringham Shoal's. At least 15 of the 82
     located offshore rows got an answer that is a structure in the water.

     Those are not distant answers, they are answers to the wrong question,
     and no search radius fixes a wrong question - a bigger circle finds
     more sea.

     NESO publishes the right one. Its Transmission Entry Capacity register
     carries a Connection Site per contracted project and the Embedded
     register does the same for the distribution-connected ones. Hornsea 3
     connects at Norwich Main. Hornsea 4 at Creyke Beck. Sofia and Dogger
     Bank C at Lackenby. Berwick Bank at Branxton. Marram at Longside.

     This is a CONTRACTUAL fact from the body that issues connections, not
     a measurement and not a route. It is printed as that, above the
     measurement, and it never replaces the measurement - both are true and
     they answer different questions.

     XMLHttpRequest, not fetch, and deliberately: the composer executes
     cartridges from a blob: origin where fetch resolves 200 and then throws
     TypeError reading the body. Measured in this Atlas, twice. */
  let nesoPromise = null;
  function loadNesoSites() {
    if (nesoPromise) return nesoPromise;
    nesoPromise = new Promise((resolve) => {
      try {
        const url = new URL('../../data/neso-connection-sites.lean.json', document.baseURI).href;
        ...
        xhr.onload = () => {
          ...
          try {
            ...
            link.neso_sites_loaded = Object.keys(payload.sites || {}).length;
            resolve(payload.sites || {});
          } catch (_) { resolve({}); }
        };
        // An absent file means no declared connection is known, which is a
        // state this engine already knows how to say out loud.
        xhr.onerror = () => resolve({});
        xhr.send();
      } catch (_) { resolve({}); }
    });
  ...
  /* A connection site is a place name wearing a voltage and the word
     substation. Both sides are reduced the same way or "Norwich Main
     Substation" in the payload never meets "Norwich Main 400kV Substation"
     in the register - which was a real bug in the join that built this
     file, and it made 30 placeable sites look unplaceable. */
  function siteKey(value) {
    return String(value || '')
      .toLowerCase()
      .replace(/\d+\s*\/?\s*\d*\s*kv/g, ' ')
      .replace(/[^a-z0-9 ]/g, ' ')
      .replace(/\b(substation|substations|gsp|grid supply point|node|platform|offshores?|the)\b/g, ' ')
      .replace(/\s+/g, ' ')
      .trim();
  }

  async function resolveNesoConnection(repdRef, origin, subs) {
    if (!repdRef) return null;
    let sites = {};
    try { sites = await loadNesoSites(); } catch (_) { return null; }
    const entry = sites[String(repdRef)];
    if (!entry || !entry.site) return null;
    const key = siteKey(entry.site);
    let bound = null;
    if (key) {
      for (const sub of subs || []) {
        if (siteKey(sub.name) !== key) continue;
        // Prefer the highest-voltage feature carrying the name: a named site
        // often has several, and the connection is made at the top class.
        if (!bound || (sub.kv?.[0] || 0) > (bound.kv?.[0] || 0)) bound = sub;
      }
    }
    const out = {
      site: entry.site,
      neso_name: entry.neso_name || null,
      register: entry.register || null,
      confidence: entry.confidence || null,
      mw: Number.isFinite(Number(entry.mw)) ? Number(entry.mw) : null,
      status: entry.status || null,
      sites: Array.isArray(entry.sites) ? entry.sites : null,
      at: bound ? bound.at : null,
      kv: bound && bound.kv && bound.kv.length ? bound.kv[0] : null,
      km: null,
    };
    if (bound && origin) out.km = distanceKm(origin[0], origin[1], bound.at[0], bound.at[1]);
    return out;
  }
  ...
  /* The connection site, printed as the contractual fact it is.
     ---------------------------------------------------------------------
     First on the card, because for an offshore project it is the answer to
     the question the reader actually asked, and every distance below it is
     a different question. It never replaces a measurement: where the site
     is in the payload the straight-line distance to it is given, and where
     it is not the card says the substation is not in this map's data rather
     than quietly substituting the nearest thing that is. Several of the
     misses are substations that do not exist yet - Longside, Friston,
     Birkhill Wood, Hurlie, Banniskirk, Brechin - which is a fact about the
     grid, not a gap in the map. */
  function nesoBlockHtml() {
    const n = currentNeso;
    if (!n || !n.site) return '';
    const many = Array.isArray(n.sites) && n.sites.length > 1;
    const measured = n.at && Number.isFinite(n.km);
    return `<div class="neon-hd">Declared connection`
      + (n.confidence === 'MEDIUM'
        ? `<span class="neon-beta" style="background:#d87aa8;color:#1a0b13">Named by inference</span>` : '')
      + `<span class="neon-beta">NESO register</span></div>`
      + `<ol><li>`
      + (measured ? `<span class="neon-km">${n.km.toFixed(2)} km</span>` : '')
      + `<span class="neon-name">${escapeHtml(n.site)}</span>`
      + (n.kv ? `<span class="neon-kv">${n.kv} kV</span>` : '')
      + `</li></ol>`
      + `<p class="neon-caveat">`
      + `The substation this project is contracted to connect at, from NESO's `
      + `${escapeHtml(n.register === 'EMBEDDED' ? 'Embedded' : 'Transmission Entry Capacity')} register`
      + (n.neso_name && siteKey(n.neso_name) !== siteKey(n.site)
        ? `, which lists it as ${escapeHtml(n.neso_name)}` : '')
      + `. This is a contractual fact from the connection authority, not a `
      + `measurement and not a cable route.`
      + (n.mw ? ` Contracted capacity ${n.mw.toLocaleString('en-GB')} MW`
        + (n.status ? `, ${escapeHtml(String(n.status).toLowerCase())}` : '') + '.' : '')
      + (measured
        ? ` The distance shown is a straight line from this project to that `
          + `substation, not the length of any cable.`
        : ` That substation is not in this map's substation data, so no `
          + `distance is measured to it and no line is drawn. Several declared `
          + `connection points are substations that have not been built yet.`)
      + (many
        ? ` The register holds more than one connection site for this project: `
          + `${escapeHtml(n.sites.join('; '))}.` : '')
      + (n.confidence === 'MEDIUM'
        ? ` The register names this project differently from the register of `
          + `planning applications, so the two were paired by name and capacity `
          + `rather than by a shared identifier.` : '')
      + `</p>`;
  }

  function declaredBlockHtml(toSubstations) {
    if (!toSubstations) return '';
    let out = nesoBlockHtml();
    if (currentDeclared) {
  ...
  const link = {
    ...
    last_selection: null,
    /* Which cable engine answered this arrival, whether it answered, and what
       it fell back from. Present from boot so a reader can tell "not yet
       asked" from "asked and silent". */
    arrival_engine: null,
    links_drawn: 0,
    deep_linked: false,
    boot_trigger: null,
  ...
  function invalidatePendingArrival(reason) {
    ...
    /* The gate has moved, so the standing answer belongs to an arrival that is
       no longer the one on screen. Leaving it was the difference between a
       silent map and a map that reports the previous project's answer as this
       project's: click A, get an answer; click B while the substation payload
       is failing, and A's green sentinel is still sitting there. Every path
       that moves the gate comes through here - clear, history navigation,
       new selection, identity retry - so this is the one place it belongs. */
    try { clearArrivalEngine(); } catch (_) { link.arrival_engine = null; }
    // The declared frame belongs to the arrival that drew it, and no other.
    if (typeof declaredFrameGuard === 'function') { try { declaredFrameGuard(); } catch (_) {} }
    link.declared_connection_framed = false;
    try {
  ...
  function loadSubstations() {
    ...
    substationsPromise = (async () => {
      ...
      // published so the interconnectors module can measure the GB converter
      // against the same list the project arrival uses
      link.substations = out;
      return out;
    })().catch(error => {
  ...
  function nearestSubstations(lon, lat, subs) {
    const scored = [];
    /* Beyond MAX_LINK_KM nothing is drawn, and that stays true. What changes is
       that the nearest one is still measured. Returning an empty list with a
       null distance says "no grid data here", when the fact is "the nearest
       mapped substation is further than the draw radius" - a different
       statement, and the only true one. Measured on Bowdun, Hornsea 3 and
       Berwick Bank, which each resolve and map correctly and then report
       nothing at all.

       This follows the reasoning already written into the offshore branch of
       this file: the distance to the nearest mapped substation is a measurement
       of a real thing, and withholding it was over-caution. */
    let beyond = null;
    for (const sub of subs) {
      const km = distanceKm(lon, lat, sub.at[0], sub.at[1]);
      if (km > MAX_LINK_KM) {
        if (!beyond || km < beyond.km) beyond = { ...sub, km };
        continue;
      }
      scored.push({ ...sub, km });
    }
    scored.sort((a, b) => a.km - b.km);
    const result = scored.slice(0, LINK_COUNT);
    // Carried on the array so no caller has to change shape to read it.
    result.beyondRadiusNearest = result.length ? null : beyond;
    result.radiusKm = MAX_LINK_KM;
    return result;
  }
  ...
  function cardBlockHtml(links, direction, layerLoaded = true) {
    ...
    if (!links.length) {
      /* THE MEASUREMENT EXISTS; PRINT IT.
         ------------------------------------------------------------------
         nearestSubstations() already carries the nearest substation OUTSIDE
         the draw radius on `links.beyondRadiusNearest`, and until now it went
         only into telemetry: arrival_engine said answered, last_selection
         carried the kilometres and the name, and the card said "no mapped
         substation within 40 km" - so the record and the reader disagreed
         about the same arrival, and the reader was told the smaller truth.

         It also contradicted the block directly above it, which prints an
         unbounded nearest 400 kV line: "no substation within 40 km" sitting
         over "nearest 400 kV substation: Peterhead, 222.9 km" reads as a
         broken map rather than a bounded one.

         This is the whole of the offshore silence. Fourteen of the eighty-two
         located offshore wind rows have nothing at all inside 40 km - Dogger
         Bank, Hornsea 1-4, Berwick Bank, Bowdun, Sofia, Culzean among them -
         and every one of them had a real distance to a real named substation
         sitting in memory while the card claimed there was nothing.

         The radius is NOT widened. A 217 km animated line from Dogger Bank to
         Denwick would be a picture of a cable that does not exist. The number
         is given, the line is not drawn, and the card says which. */
      const beyond = toSubstations ? (links.beyondRadiusNearest || null) : null;
      const nothing = toSubstations
        ? (beyond
          ? `Nothing within ${MAX_LINK_KM} km, so no line is drawn. The nearest `
            + `mapped substation at ${MIN_KV} kV or above is `
            + `${escapeHtml(beyond.name || 'unnamed')}`
            + `${beyond.kv && beyond.kv.length ? ` (${beyond.kv[0]} kV)` : ''}, `
            + `${beyond.km.toFixed(1)} km straight line.`
      ...
      /* An offshore project's export cable lands somewhere before it reaches
         any of this, and where it lands is a published fact this estate does
         not yet hold. Say that, rather than letting a straight line over water
         be read as a route. */
      const offshoreRoute = (toSubstations && beyond && currentPolicy?.offshore)
        ? `<p class="neon-caveat">This is a straight line from the array, not a `
          + `route. An export cable lands at a declared onshore point, and that `
          + `declaration is not in this estate's data, so the landfall is not `
          + `measured and no connection is claimed.</p>`
        : '';
      return `<div class="${BLOCK_CLASS}">${declaredHtml}${head}`
        + `<div class="neon-caveat">${nothing}</div>${offshoreRoute}${policyNotesHtml()}`
        + `${caveatHtml()}</div>`;
    }
  ...
  function boundCardToMap() {
    try {
      ...
      if (popup.classList.contains('gridatlas-free')) {
        ...
        content.style.setProperty('max-height', Math.max(120, Math.min(available, map.height - 48)) + 'px', 'important');
        return;
      }
      const rect = popup.getBoundingClientRect();
      const available = map.bottom - rect.top - 12;
      if (rect.top < map.top + 12 || rect.right > map.right - 12 || rect.left < map.left + 12 || available < MIN_ANCHORED_CARD) {
        ...
        content.style.setProperty('max-height', Math.max(160, map.height - 48) + 'px', 'important');
        return;
      }
      content.style.setProperty('max-height', Math.min(available, map.height - 48) + 'px', 'important');
  ...
  function resetCardGeometry(content, linksDrawn = null) {
    const popup = content?.closest?.('.maplibregl-popup');
    if (!popup) return;
    popup.classList.remove('gridatlas-free');
    /* GG-029, GG-027. On a phone the card docks as a full-width sheet over the
       bottom of the map, which is where the engine draws its lines and where
       the layers panel opens. Arriving expanded therefore hid both: the owner
       photographed the layers label flipping with nothing else changing,
       because the panel was opening behind this sheet.

       So on a sheet target the card arrives MINIMISED: its bar, its title and
       its restore control, and nothing else. The reader expands it when they
       want the numbers. Anywhere with room, the card opens as it always did. */
    /* But minimising assumes there is something on the map worth uncovering.
       When the engine answers from beyond the draw radius no lines are drawn,
       so the sheet covers an empty map and the card it hides is the ONLY place
       the answer exists. That is what "it didn't fire" looked like on the
       owner's phone for Berwick Bank: the camera moved, no neon appeared, and
       the one sentence carrying 52.0 km to Neart na Gaoithe was behind a
       collapsed bar. So: minimised when there are lines to see, open when the
       card is the whole answer. */
    const arriveMinimised = (function () {
      /* v9.150 opened the card when nothing was drawn, on the reasoning that
         the card was then the whole answer. The owner's ruling: the card is
         minimised for a project, always - the map is the answer and the sheet
         is the footnote. v9.152 makes that true again by drawing the declared
         connection, so nothing is behind the plus sign that is not also on the
         map. `linksDrawn` is kept in the signature for the callers. */
      void linksDrawn;
      try { return Boolean(sheetTarget()); } catch (_) { return false; }
    })();
    if (arriveMinimised) popup.classList.add('gridatlas-min');
    else popup.classList.remove('gridatlas-min');
    popup.style.removeProperty('--gx');
    popup.style.removeProperty('--gy');
    content.style.removeProperty('max-height');
    ...
    if (toggle) toggle.innerHTML = arriveMinimised ? '&plus;' : '&minus;';
  }
  ...
  function injectIntoCard(links, direction, layerLoaded = true) {
    ...
    resetCardGeometry(content, links.length);
    addCardBar(content);
  ...
  function injectDeclaredOnly() {
    ...
    try {
      installStyles();
      /* A declared-only card carries no measured lines by definition, so it is
         the whole answer too. */
      resetCardGeometry(content, 0);
      addCardBar(content);
      const holder = document.createElement('div');
  ...
  /* One writer for the sentinel, at module scope.
     ---------------------------------------------------------------------
     v9.148 introduced `arrival_engine` and wrote it from two places: a
     literal inside drawLinks() and a helper inside install(). Two writers of
     one record is a maintenance trap - an epoch added to one and not the
     other makes the success path and the failure paths disagree - and, worse,
     neither writer existed on the paths where the engine never installs at
     all, which is precisely the "it did not launch" case. So the record has
     one writer, it lives at module scope where every path can reach it, and
     install()'s answerArrival() delegates to it rather than duplicating it.

     It also mirrors to the DOM. A sentinel readable only from the JavaScript
     console is not a surface anyone can open, and the whole point of this
     record is that a person - not just a test - can tell a silent map from a
     broken one. */
  function recordArrivalEngine(engine, answered, reason, fallbackFrom, pending = false) {
    let epoch = null;
    try { epoch = arrivalGate.snapshot().epoch; } catch (_) { epoch = null; }
    /* `reason` is not "why it failed", it is "what happened", and it survives a
       true answer. An arrival answered from beyond the draw radius answered -
       there is a real distance and a real named substation - and it is also a
       materially different answer from one with five lines on the map. v9.148
       carried that distinction and nulling the reason on success would throw it
       away. An unanswered arrival with nothing to say is UNSTATED, which is a
       defect, not a state to be tolerated quietly. */
    /* `terminal` separates "this engine has finished with this arrival" from
       "this engine has taken the arrival and is still working". Without it a
       handover looks exactly like a refusal: an interconnector arrival is
       passed to the interconnector module in the first milliseconds, and a
       reader - or a test - that stops at the first record written would call
       every interconnector in the country unanswered while its measurement was
       still running. A non-terminal record is a receipt, not a verdict. */
    link.arrival_engine = {
      engine: engine || 'onshore',
      answered: Boolean(answered),
      reason: reason || (answered ? null : 'UNSTATED'),
    ...
    try {
      capturedMap?.getContainer?.().setAttribute('data-gridatlas-arrival',
        link.arrival_engine.answered ? 'answered' : String(link.arrival_engine.reason));
      capturedMap?.getContainer?.().setAttribute('data-gridatlas-arrival-engine',
        String(link.arrival_engine.engine));
    } catch (_) { /* the object on window is the record; the attribute is a convenience */ }
    return link.arrival_engine;
  }
  link.measure.recordArrivalEngine = recordArrivalEngine;

  function clearArrivalEngine() {
    link.arrival_engine = null;
    try {
      capturedMap?.getContainer?.().removeAttribute('data-gridatlas-arrival');
      capturedMap?.getContainer?.().removeAttribute('data-gridatlas-arrival-engine');
  ...
  function drawLinks(map, origin, name, tech, links, direction, statedMw, layerLoaded = true) {
    ...
    /* THE DECLARED CONNECTION IS DRAWN, NOT ONLY PRINTED.
       ------------------------------------------------------------------
       v9.151 read the NESO register and put the connection site on the card,
       and the owner opened Berwick Bank on his phone and saw a dot in the sea
       with a collapsed sheet under it: "didn't fire". He was right. The engine
       had answered - Branxton, 80.57 km - and the answer was a sentence behind
       a plus sign while the map showed nothing at all, because the rule since
       v9.14x is that nothing beyond the 40 km draw radius is drawn.

       That rule exists so a 217 km straight line to whatever happens to be
       nearest is never mistaken for a cable. A declared connection is not
       that. It is the substation the connection authority has contracted this
       project to, so a line to it is a picture of a real thing - drawn in the
       declared gold rather than the neon of a measurement, and labelled as the
       declared connection, so it can never read as one more nearest-neighbour
       result. It stays a straight line and the card still says so. The owner
       then measured it himself along the water via Neart na Gaoithe: 88.56 km
       against this line's 80.57 km straight, which is the difference between a
       route and a chord, and the card names it as a chord. */
    let nesoDrawn = false;
    if (direction === 'to-substation' && currentNeso?.at && !currentDeclared?.at
        && Number.isFinite(currentNeso.km)) {
      lines.push({ type: 'Feature',
        properties: { colour: DECLARED_COLOUR, strength: 0.85, km: currentNeso.km },
        geometry: { type: 'LineString', coordinates: [origin, currentNeso.at] } });
      nodes.push({ type: 'Feature',
        properties: { colour: DECLARED_COLOUR,
          label: 'Declared \u00b7 ' + currentNeso.km.toFixed(2) + ' km'
            + (currentNeso.kv ? ' \u00b7 ' + currentNeso.kv + ' kV' : '') },
        geometry: { type: 'Point', coordinates: currentNeso.at } });
      nesoDrawn = true;
    }
    ...
    /* And framed. A line whose far end is 80 km outside the viewport is as
       invisible as no line. When the declared connection is the only thing
       drawn, the camera fits both ends; when nearest links are drawn too the
       reader already has something in view and the camera is left alone. The
       bottom padding keeps the far end above a phone's docked card. */
    if (nesoDrawn && !links.length) {
      try {
        let sheet = false;
        try { sheet = Boolean(sheetTarget()); } catch (_) { sheet = false; }
        map.fitBounds(
          [[Math.min(origin[0], currentNeso.at[0]), Math.min(origin[1], currentNeso.at[1])],
           [Math.max(origin[0], currentNeso.at[0]), Math.max(origin[1], currentNeso.at[1])]],
          { padding: { top: 90, left: 40, right: 40, bottom: sheet ? 220 : 90 },
            maxZoom: 10, duration: 900, essential: true });
        link.declared_connection_framed = true;
        /* And kept. The identity lane resolves the project against the
           register over the network and flies to it at zoom 12 when it does -
           1.2 s, essential - and on a real connection that lands AFTER this
           frame, not before. Measured on Hornsea 3 in Chromium on the live
           root: framed at zoom 7.06, then carried to 11.17 on the array with
           Norwich Main gone off the bottom. Locally the register answered first
           and the race never showed, which is the usual way a race hides.

           So the frame re-asserts itself, at most twice and only within the
           arrival window, if the camera leaves it without a person touching the
           map. MapLibre sets originalEvent on user-initiated moves and leaves
           it undefined on programmatic ones, and that is the whole test: a
           reader who drags or pinches owns the camera from that moment on. */
        const both = [
          [Math.min(origin[0], currentNeso.at[0]), Math.min(origin[1], currentNeso.at[1])],
          [Math.max(origin[0], currentNeso.at[0]), Math.max(origin[1], currentNeso.at[1])]];
        const holds = () => {
          try { const b = map.getBounds(); return b.contains(both[0]) && b.contains(both[1]); }
          catch (_) { return true; }
        };
        /* v9.153 held this guard for twelve seconds and it never fired on the
           live root: a cold DuckDB identity boot over a real connection takes
           longer than that, so the late flight landed after the listeners were
           gone. Measured in Chromium on Berwick Bank and Hornsea 3, both ending
           at zoom 11 on the array with the far end lost. A clock is the wrong
           shape for this. The guard now lives as long as the arrival does - it
           is detached when the arrival gate moves, by clearLinks or a new
           selection - and stands down the moment a person takes the map.

           Two kinds of person. A drag or a pinch arrives with originalEvent on
           the map's movestart. A tap on a control that then moves the camera
           programmatically - the layout button, the scope tools - does not, so
           any pointer, touch, key or wheel anywhere in the document within the
           last three seconds is also read as the reader taking over. Otherwise
           the frame would win a fight against a button the reader just
           pressed, and that is the one fight it must lose. */
        if (typeof declaredFrameGuard === 'function') { try { declaredFrameGuard(); } catch (_) {} }
        let userOwned = false, reasserted = 0, lastInput = 0;
        const noteInput = () => { lastInput = Date.now(); };
        const onStart = (e) => { if (e && e.originalEvent) userOwned = true; };
        const onEnd = () => {
          if (userOwned || Date.now() - lastInput < 3000 || reasserted >= 2 || holds()) return;
          reasserted += 1;
          link.declared_frame_reasserted = reasserted;
          try {
            map.fitBounds(both, { padding: { top: 90, left: 40, right: 40, bottom: sheet ? 220 : 90 },
              maxZoom: 10, duration: 600, essential: true });
          } catch (_) { /* the first frame stands */ }
        };
        const inputEvents = ['pointerdown', 'touchstart', 'keydown', 'wheel'];
        inputEvents.forEach(t => document.addEventListener(t, noteInput, { capture: true, passive: true }));
        map.on('movestart', onStart);
        map.on('moveend', onEnd);
        declaredFrameGuard = () => {
          try { map.off('movestart', onStart); map.off('moveend', onEnd); } catch (_) { /* gone */ }
          inputEvents.forEach(t => { try { document.removeEventListener(t, noteInput, { capture: true }); } catch (_) {} });
          declaredFrameGuard = null;
        };
      } catch (error) {
        link.failures.push('declared frame: ' + String(error?.message || error));
      }
    }
    ...
    /* The engine has finished. Offshore is a different question from onshore -
       an export cable lands at a declared point - and until that declaration
       exists offshore is answered by the onshore engine. That is a legitimate
       fallback and it is named as one rather than passed off as the answer. */
    if (direction !== 'from-substation') {
      let offshore = false;
      try { offshore = Boolean(coverage.policy(tech)?.offshore); } catch (_) { offshore = false; }
      recordArrivalEngine(
        offshore ? 'offshore' : 'onshore',
        links.length > 0 || Boolean(links.beyondRadiusNearest),
        links.length ? null
          : (nesoDrawn ? 'DECLARED_CONNECTION_DRAWN'
            : (links.beyondRadiusNearest ? 'NEAREST_BEYOND_' + MAX_LINK_KM + '_KM' : 'NO_SUBSTATION_DATA')),
        /* Only a fallback while nothing declares the connection. Now that the
           NESO register is read, an offshore project with a connection site is
           answered by its declared point and says so, and only a project with
           neither is falling back to a nearest-substation search. */
        offshore && !currentDeclared && !currentNeso?.site ? 'declared_connection_point' : null
      );
    }
    /* Two different facts, never collapsed into one null. `nearest_km` is the
       nearest mapped substation whether or not it was close enough to draw;
       `nearest_beyond_radius` says which of the two it is, so a reader can tell
       "nothing within the draw radius" from "no substation data at all". */
    const beyond = links.length ? null : (links.beyondRadiusNearest || null);
    link.last_selection = {
      name, tech, direction, count: links.length,
      nearest_km: links.length
        ? Number(links[0].km.toFixed(3))
        : (beyond ? Number(beyond.km.toFixed(3)) : null),
      nearest_beyond_radius: links.length ? false : Boolean(beyond),
      nearest_name: links.length ? (links[0].name || null) : (beyond ? (beyond.name || null) : null),
      draw_radius_km: links.radiusKm != null ? links.radiusKm : null,
    };
  ...
  /* THE STATUS BOX IS READ, OR IT IS NOT A STATUS. It sat at a fixed top:14px inside the map, and the menu bar, the
     search box and the search results all lie over the top of the map: on a phone the placeholder sentence was printed
     underneath three other things and a reader got its last line. It was also 195 px wide on a 390 px screen, because
     a box centred with left:50% can only ever be as wide as the half of the map to its right.

     So it is given the width of the screen less a margin, and it finds its own height: it starts where it always did
     and moves down until the page itself says that a finger on its centre and on each of its corners would touch the
     box and nothing else. No selector is named, because the furniture above it has changed before and will again. It
     asks again after the search results have had time to appear, and when the window changes shape. */
  function placeStatus(el) {
    try {
      if (!document.getElementById(STATUS_ID + '-place')) {
        const style = document.createElement('style');
        style.id = STATUS_ID + '-place';
        style.textContent = '#' + STATUS_ID + '{left:4vw;right:4vw;transform:none;margin:0 auto;'
          + 'width:fit-content;max-width:min(92vw,420px);box-sizing:border-box}';
        document.head.appendChild(style);
      }
      const clear = () => {
        const r = el.getBoundingClientRect();
        if (r.width < 2 || r.height < 2) return true;
        /* A GRID of points, not the centre and the corners: the search box lies across the lower middle of this
           box, between all five of those, and the first version of this rule declared the box clear with a text
           field printed straight over its sentence. Nine columns by five rows leaves no gap a control can hide in. */
        const points = [];
        for (let i = 0; i < 9; i++) for (let j = 0; j < 5; j++) {
          points.push([r.left + 3 + (r.width - 6) * i / 8, r.top + 3 + (r.height - 6) * j / 4]);
        }
        return points.every(([x, y]) => {
          const hit = document.elementFromPoint(Math.min(Math.max(x, 1), innerWidth - 1),
      ...
      const settle = () => {
        ...
        let top = 14;
        el.style.top = top + 'px';
        const limit = Math.max(160, innerHeight * 0.6);
        while (!clear() && top < limit) { top += 8; el.style.top = top + 'px'; }
        link.status_top_px = top;
      };
      settle();
      for (const ms of [300, 1200, 3000, 6000]) setTimeout(settle, ms);
      if (!placeStatus.listening) {
        placeStatus.listening = true;
        window.addEventListener('resize', () => {
          const live = document.getElementById(STATUS_ID);
          if (live) placeStatus(live);
        });
      }
    } catch (error) {
      link.failures.push('status placement: ' + String(error?.message || error));
    }
  }
  ...
  function install(map) {
    ...
    async function selectAt(origin, name, tech, fromSubstation, statedMw,
      ...
      if (fromSubstation) {
        ...
        drawLinks(map, origin, name, tech, found.links, 'from-substation',
          null, found.loaded);
        /* drawLinks deliberately skips the sentinel for this direction, because
           the engine class is read from the project's technology and a
           substation has none. The question was still asked and still answered,
           so it is named here with its own class rather than left blank and
           counted as an engine that went quiet. */
        try {
          answerArrival('substation-origin', found.links.length > 0,
            found.links.length ? null : 'NO_PROJECTS_NEAR_SUBSTATION', null);
        } catch (_) {}
        return true;
      }
      ...
      if (!subs.length) {
        ...
        showStatus('The substation data did not load, so no links can be '
          ...
          'failed');
        /* The single terminus for every network failure on this path:
           loadSubstations() catches its own rejection and returns [], so a
           dead CDN, a blocked fetch and a corrupt payload all arrive here.
           It was also the highest-traffic way an arrival ended with nothing
           on the record. `return false`, not a bare return, because a bare
           return is undefined and the caller reads undefined as success. */
        try {
          answerArrival(classifyEngine(String(tech || ''), null), false,
            'SUBSTATION_PAYLOAD_ABSENT', null);
        } catch (_) {}
        return false;
      }
      clearStatus();
      ...
      /* Awaited before the card is drawn, because a connection the authority
         has issued is the headline fact and arriving after the measurement
         would make it look like an afterthought. The file is 130 KB and
         cached, so this costs one request per session. */
      try { currentNeso = await resolveNesoConnection(currentRepdRef, origin, subs); }
      catch (_) { currentNeso = null; }
    ...
    /* The class selects the question. It never selects whether a question is
       answered: every arrival leaves one of these behind, and an engine that
       cannot answer names its reason instead of going quiet. */
    function classifyEngine(tech, q) {
      try {
        if (q && (q.get('interconnector') || String(q.get('technology') || '') === 'interconnector')) return 'interconnector';
      } catch (_) { /* no query is not an interconnector */ }
      try { return coverage.policy(tech)?.offshore ? 'offshore' : 'onshore'; }
      catch (_) { return 'onshore'; }
    }
    /* Delegates. There is one writer of this record and it lives at module
       scope, because the paths that most need it - install() throwing, no
       style at all - are outside install() and cannot see anything declared
       in here. Two writers would drift, and the epoch and the DOM mirror
       would end up on one of them. */
    function answerArrival(engine, answered, reason, fallbackFrom, pending = false) {
      return recordArrivalEngine(engine, answered, reason, fallbackFrom, pending);
    }
    link.measure.answerArrival = answerArrival;
    link.measure.classifyEngine = classifyEngine;
    ...
    map.on('click', async (event) => {
      try {
        ...
        if (!ids.length) {
          /* A click on a project the reader can plainly see, at a moment when
             no interactive layer is registered - the layers are still off, or
             the style is being repopulated. Nothing happens and nothing is
             said, which reads as a dead map. Only written when no answer is
             standing, so a working answer is never overwritten by a stray
             click on the basemap. */
          if (!link.arrival_engine) {
            try { answerArrival('onshore', false, 'NO_INTERACTIVE_LAYERS', null); } catch (_) {}
          }
          return;
        }
        let features = [];
        try { features = map.queryRenderedFeatures(event.point, { layers: ids }); }
          ...
          try { answerArrival('onshore', false, 'QUERY_RENDERED_FEATURES_THREW', null); } catch (_) {}
          return;
        }
        ...
        /* A click that throws leaves the reader looking at an unchanged map.
           The failure was already recorded where only a developer would find
           it; the engine now names it too. */
        try { answerArrival('onshore', false, 'MAP_CLICK_THREW', null); } catch (_) {}
        link.failures.push(String(error?.message || error));
      }
    });
  ...
  async function waitForResolvedIdentity(options = {}) {
    ...
    for (;;) {
      ...
      if (dl) {
        ...
        /* RESOLVED_UNMAPPED was missing from this list, and it is terminal: the
           identity lane has finished, found the project in the register, and
           established that neither REPD nor a Crown Estate lease gives it a
           location. Treating it as "still working" made this loop spin for its
           full ten minutes and then report an identity failure that had not
           happened - so the page sat at the default GB view saying nothing at
           all. Measured on REPD 13432 (Marram), reported live by the owner:
           "Didn't even go to anything". */
        if (dl.status === 'FAILED' || dl.status === 'ABSENT'
            || dl.status === 'NOT_IN_ACTIVE_REGISTER'
            || dl.status === 'RESOLVED_UNMAPPED'
            || dl.status === 'CANCELLED') {
          if (told) clearStatus();
          return dl;
        }
    ...
    async function runDeepLink(expectedArrivalEpoch = null) {
      try {
        ...
        /* An interconnector arrival is not a project arrival. Pipeline News'
           INTERCONNECTORS tab sends interconnector=<BMRS>&technology=
           interconnector with a label anchor - the node drawn mid-line, or
           the GB converter - and measuring a generator from that point would
           report the sea (a BritNed midpoint is 59.2 km from the nearest
           substation). The interconnectors module owns this arrival: it
           frames the span and measures each converter within its own search
           budget. This lane records the handover and does nothing else. */
        if (q.get('interconnector') || String(q.get('technology') || '') === 'interconnector') {
          link.arrival_reconciliation = {
            status: 'HANDED_TO_INTERCONNECTORS',
            interconnector: q.get('interconnector') || null,
            statement: 'interconnector arrival; the project engine does not measure from a label anchor'
          };
          /* v9.148 recorded this handover as a comment reading "answered by the
             interconnector engine, not unanswered" and wrote nothing. It was
             the only claim in that version that was not implemented, and it
             left every interconnector arrival with arrival_engine === null in
             every branch - indistinguishable from an engine that never ran.

             So the handover writes the record itself: the interconnector engine
             owns this arrival and has not answered yet. The interconnectors
             module overwrites it with the real outcome, and if that module
             never boots, this stands and says which engine went quiet. */
          recordArrivalEngine('interconnector', false, 'HANDED_TO_INTERCONNECTORS', null, true);
          // the interconnectors module measures the GB converter against this payload
          try { void loadSubstations(); } catch (_) { /* best effort; the module reports absent coverage honestly */ }
          return false;
        }
        ...
        function honourRequestedZoom(map) {
          ...
          const apply = () => {
            ...
            try {
              map.off('idle', apply);
              /* The link asked for a zoom; the declared connection asked for a
                 frame that holds both ends. The frame wins, because a zoom the
                 link chose before it knew the far end was 80 km away would cut
                 that end off - measured in Chromium on Hornsea 3, where this
                 eased in from the fitted view and pushed the array above the
                 top edge. WebKit only escaped by timing. */
              if (link.declared_connection_framed) {
                link.zoom_applied = 'declared frame kept';
                return;
              }
        ...
        /* Null Island is not the only placeholder. The register carries a
           second one - 49.766807 N, 7.55716 W, in the Western Approaches
           south-west of the Scillies - on fifteen of its ninety-seven offshore
           wind rows, among them Dogger Bank South East and West, Morgan,
           Ossian, Muir Mhor and North Falls. The search cartridge has rejected
           it since v9.145 as FALSE_ORIGIN; this engine did not, so a project in
           the North Sea could be measured to Isles Of Scilly Primary
           Substation and the distance printed as fact. A false measurement is
           worse than a stated silence. */
        const FALSE_ORIGIN = { longitude: -7.55716, latitude: 49.766807 };
        const isFalseOrigin = (x, y) => Math.abs(x - FALSE_ORIGIN.longitude) < 1e-4
          && Math.abs(y - FALSE_ORIGIN.latitude) < 1e-4;
        const coordsUsable = () => Number.isFinite(lon) && Number.isFinite(lat)
          && Math.abs(lon) <= 180 && Math.abs(lat) <= 90
          && !(Math.abs(lon) < 1e-9 && Math.abs(lat) < 1e-9)
          && !isFalseOrigin(lon, lat);
        ...
        if (receiverPlan.route === 'MEASURE_LINK_FIRST' && repdRef) {
          ...
          /* Split, because these are two different facts wearing one guard. A
             stale epoch means a newer arrival has taken the map and is about to
             publish its own answer, so this one must say nothing or it would
             overwrite a live answer with a dead one. A CANCELLED owner means
             this arrival really did end, and that has to be named. */
          if (!arrivalGate.isCurrent(epoch)) return;
          if (owner?.status === 'CANCELLED') {
            try { answerArrival(classifyEngine(String(tech || ''), q), false, 'IDENTITY_OWNER_CANCELLED', null); } catch (_) {}
            return;
          }
          if (owner?.status === 'RESOLVED') {
          ...
          } else if (owner?.status === 'RESOLVED_UNMAPPED') {
            /* The register knows this project and cannot place it. That is a
               different fact from "not in the register" and a different fact
               again from "the identity check failed", and it deserves its own
               words: the name, capacity and reference are real and are shown;
               the location is absent, so nothing is measured and nothing is
               claimed. Thirteen offshore wind rows are in this state today,
               including Marram, Muir Mhor and Morecombe.

               The engine is not silent about it and the camera does not move,
               because moving it would put the reader somewhere that is not the
               project. */
            link.origin_source = 'register-resolved-without-a-coordinate';
            link.deep_link_identity = 'terminal-resolved-unmapped';
            link.identity_verification = {
              status: 'RESOLVED_UNMAPPED', repd_ref: repdRef,
              official_active_register_match: true,
              supplied_coordinates_kept: false,
              coordinate_source: 'NONE'
            };
            injectStatusStyle();
            const known = [owner?.name ? String(owner.name) : null,
              Number.isFinite(Number(owner?.capacity_mw)) && Number(owner.capacity_mw) > 0
                ? Number(owner.capacity_mw).toLocaleString('en-GB') + ' MW' : null,
              owner?.status_value ? String(owner.status_value) : null]
              .filter(Boolean).join(' · ');
            showStatus((known ? known + '. ' : '')
              + 'REPD ' + repdRef + ' is in the active register and publishes no '
              + 'coordinate for it, and no seabed lease locates it either. The '
              + 'identity is confirmed; the location is not, so no distance is '
              + 'measured and no grid connection is implied.',
              'unavailable');
            try { answerArrival(classifyEngine(String(tech || ''), q), false, 'REGISTER_PUBLISHES_NO_COORDINATE', 'derived_lease_point'); } catch (_) {}
            return;
            ...
            showStatus('REPD ' + repdRef + ' is not in the active-register '
              ...
              'unavailable');
            try { answerArrival(classifyEngine(String(tech || ''), q), false, 'NOT_IN_ACTIVE_REGISTER_NO_SUPPLIED_POINT', null); } catch (_) {}
            return;
          } else {
            injectStatusStyle();
            ...
            try { answerArrival(classifyEngine(String(tech || ''), q), false, 'IDENTITY_CHECK_FAILED', null); } catch (_) {}
            return;
          }
        } else {
          link.origin_source = 'link-supplied';
        ...
        if (!coordsUsable()) {
          /* A PLAIN VISIT SAYS NOTHING. Vikram, 2026-09-20: "gridatlas still has random text near the top when no
             project is loaded". This branch ran for every visitor, so an address with no project in it at all was
             told that no usable coordinate had reached the map for "this project". The refusal below is right when a
             link arrived and its coordinate could not be used, and only then. With nothing in the address that names
             a project, a place or a technology, nobody asked, so nothing is said; the record says so in its own word
             rather than reporting a failed project that never existed. */
          const projectKeys = ['repd_ref', 'longitude', 'latitude', 'project', 'technology', 'capacity_mw', 'status'];
          if (!projectKeys.some(key => q.get(key) !== null)) {
            link.origin_source = 'none: a plain visit';
            try { answerArrival(classifyEngine('', q), false, 'NO_PROJECT_IN_ADDRESS', null); } catch (_) {}
            return;
          }
          const placeholder = Number.isFinite(lon) && Number.isFinite(lat) && isFalseOrigin(lon, lat);
          const why = placeholder ? 'PLACEHOLDER_COORDINATES_FALSE_ORIGIN' : 'NO_USABLE_COORDINATES';
          /* Refusing to measure is right and refusing in silence is not. The
             record alone is not a surface a reader can see, and a page that
             does nothing looks broken whatever the reason was. */
          injectStatusStyle();
          showStatus(placeholder
            ? 'The coordinate supplied for REPD ' + (repdRef || 'this project')
              + ' is the register’s placeholder point, not a location. '
              + 'Measuring from it would report a distance to the Isles of Scilly, '
              + 'so nothing is measured and nothing is claimed.'
            : 'No usable coordinate reached this map for '
              + (repdRef ? 'REPD ' + repdRef : 'this project')
              + ', so there is nothing to measure from.',
            'unavailable');
          try { answerArrival(classifyEngine(String(tech || ''), q), false, why, null); } catch (_) {}
          return;
        }
        ...
        async function runArrivalSelection(arrival, waitForOwnerCard = false,
          ...
          /* `selected === true`, not `!== false`. selectAt used to return bare
             undefined on two silent paths, and `undefined !== false` is true,
             so a wholly silent arrival reported itself current, runDeepLink
             returned true, and the retry lane recorded RESOLVED for an arrival
             that had measured nothing at all. Those returns are now explicit
             booleans; this reads the success value rather than everything that
             is not the failure value. */
          return selected === true && arrivalGate.isCurrent(expectedArrivalEpoch);
        }
        ...
        /* `q` and `tech` are declared inside the try, so they are not in scope
           here and classifyEngine cannot be called - reading them would throw a
           ReferenceError inside the very handler that exists to stop an arrival
           disappearing. The query is still on the URL, so read the class from
           there instead of guessing onshore. */
        let engine = 'onshore';
        try {
          const q2 = new URLSearchParams(window.location.search);
          if (q2.get('interconnector') || String(q2.get('technology') || '') === 'interconnector') engine = 'interconnector';
          else if (coverage.policy(String(q2.get('technology') || ''))?.offshore) engine = 'offshore';
        } catch (_) { engine = 'onshore'; }
        try { answerArrival(engine, false, 'ARRIVAL_THREW', null); } catch (_) {}
    ...
    function arrivalHasVisibleOutcome() {
      ...
      /* The sentinel belongs in this test. An offshore farm whose nearest
         substation lies beyond the draw radius is ANSWERED with a real
         distance and a real name, and it draws no line - so links_drawn is 0
         and, before this, every visibilitychange re-ran the whole arrival on a
         question that was already answered, up to five times. An answer is an
         outcome whether or not it put lines on the map. */
      return link.links_drawn > 0
        || link.declared_connection_framed === true
        || link.arrival_engine?.answered === true
        || link.origin_source === 'not-in-active-register-no-supplied-point';
    }
    function attemptArrival() {
      if (document.visibilityState !== 'visible') {
        /* Not a failure - the tab is not composited, so an arrival started here
           would fly a camera that never moves. It is still a state a reader can
           be sitting in, so it is named rather than left blank, and it is
           replaced the moment the tab is genuinely seen. */
        if (!link.arrival_engine) {
          try { answerArrival('onshore', false, 'DEFERRED_FOR_VISIBILITY', null); } catch (_) {}
        }
        return;
      }
      arrivalAttempts += 1;
    ...
    document.addEventListener('visibilitychange', () => {
      ...
      if (arrivalAttempts >= MAX_AUTO_ARRIVAL_ATTEMPTS) {
        /* Five attempts spent and still no outcome. The arrival is now
           permanently over, and this is the last chance anything has to say so
           - after this the map simply sits there. */
        try { answerArrival('onshore', false, 'ARRIVAL_ATTEMPTS_EXHAUSTED', null); } catch (_) {}
        return;
      }
  ...
  function attach(map) {
    if (capturedMap) {
      /* A second map was constructed and the arrival is bound to the first.
         Whatever the reader is looking at, it is not the map this engine
         measures on, and that has to be said rather than left as silence. */
      if (!link.arrival_engine) recordArrivalEngine('onshore', false, 'SECOND_MAP_NOT_CAPTURED', null);
      return;
    }
    capturedMap = map;
    ...
    const boot = () => {
      try { install(map); }
        ...
        /* install() throwing is the engine failing to exist: the click handler
           and the arrival scheduler are registered inside it, below the throw,
           so nothing downstream will ever answer. Without this the page looks
           like a map that simply found nothing. */
        recordArrivalEngine('onshore', false, 'ENGINE_INSTALL_THREW', null);
        link.failures.push(String(error?.message || error));
      }
      try { installSld(map); }
    ...
    if (map.isStyleLoaded?.()) { link.boot_trigger = 'already-loaded'; boot(); }
      ...
      setTimeout(() => {
        ...
        if (hasStyle) {
          ...
          /* This is the literal "the engine did not launch": install() is never
             called, so the click handler, the arrival scheduler and the helper
             that would name a failure do not exist. Everything downstream is
             structurally incapable of reporting, which is why this one has to
             be written here, at module scope, by hand. */
          recordArrivalEngine('onshore', false, 'ENGINE_NEVER_INSTALLED_NO_STYLE', null);
...
(function interconnectors() {
  ...
  /* The connection v9.148 built and left unplugged.
     -------------------------------------------------------------------------
     The neon-links lane publishes measure.answerArrival specifically so this
     module can name itself, and nothing ever called it. The project lane
     stands down at the interconnector guard and this lane owns the arrival
     from there, so if this module says nothing, nothing does: an
     interconnector arrival left arrival_engine null in every branch, which is
     the exact signature of an engine that never ran.

     Written through the published surface rather than by reaching into the
     other module's internals, and every call is best-effort: an engine that
     cannot record its answer must still give it. */
  function answer(ok, reason, fallbackFrom) {
    try {
      window.__GRIDATLAS_NEON_LINKS__?.measure?.answerArrival?.(
        'interconnector', Boolean(ok), reason || null, fallbackFrom || null);
    } catch (_) { /* the state object below is still the record */ }
    state.answered = { answered: Boolean(ok), reason: reason || null,
      fallback_from: fallbackFrom || null, at: new Date().toISOString() };
    return state.answered;
  }

  /* Is this arrival ours at all? A page opened with no interconnector in the
     query is not an interconnector arrival, and writing a reason for it would
     overwrite whatever the project engine is doing. */
  function isInterconnectorArrival() {
    try {
      const q = new URLSearchParams(window.location.search);
      return Boolean(q.get('interconnector'))
        || String(q.get('technology') || '') === 'interconnector';
    } catch (_) { return false; }
  }
  ...
  function searchKm() {
    ...
    if (Number.isFinite(typed) && typed > 0) {
      ...
      state.search_source = 'radius box';
      return typed;
    }
  ...
  function showLayers(map) {
    for (const id of [LINE_LAYER, NODE_LAYER]) {
      try { if (map.getLayer(id)) map.setLayoutProperty(id, 'visibility', 'visible'); } catch (_) { /* style not ready */ }
    }
  }

  function endLine(end, km) {
    if (!end) return 'not held';
    if (end.coverage === 'NONE') return end.statement;
    if (end.nearest_km === null) return 'no substation within the payload';
    return (end.nearest_name || 'nearest substation') + ' at ' + end.nearest_km + ' km'
      + (end.nearest_beyond_search ? ' (beyond the ' + km + ' km search)' : '')
      + ' · ' + end.within + ' within ' + km + ' km';
  }

  function openCard(map, link, at) {
    const gl = window.maplibregl;
    if (!gl || !gl.Popup) { state.arrival.card = 'no maplibregl.Popup'; return; }
    const km = state.search_km;
    const gb = link.ends[0], far = link.ends[1];
    const html = '<div style="font-family:monospace;background:#000;color:#fff;padding:8px 10px;min-width:280px;max-width:420px;font-size:12px;line-height:1.5">'
      + '<div style="font-size:14px"><b>' + esc(link.link) + '</b> · ' + esc(link.bmrs) + ' · interconnector</div>'
      + '<div style="color:#ffae00">' + esc(link.label) + '</div>'
      + '<div><b>GB end</b> ' + esc(endLine(gb, km)) + '</div>'
      + '<div><b>Far end</b> ' + esc(endLine(far, km)) + '</div>'
      + '<div style="color:#a6adbb">straight line converter to converter, ' + esc(link.span_km) + ' km'
      + (link.known_submarine_cable_km ? '; known submarine cable ' + esc(link.known_submarine_cable_km) + ' km' : '')
      + (link.route_factor ? '; route factor ' + esc(link.route_factor) : '') + '</div>'
      + (link.net_direction ? '<div style="color:#a6adbb">' + esc(link.net_direction) + '</div>' : '')
      + '<div style="color:#a6adbb">search budget ' + esc(km) + ' km at each end (' + esc(state.search_source) + '); widen it in the radius box</div>'
      + '</div>';
    try {
      state.arrival.popup = new gl.Popup({ closeOnClick: false, maxWidth: '440px' }).setLngLat(at).setHTML(html).addTo(map);
      state.arrival.card = 'OPEN';
    } catch (error) {
      state.arrival.card = 'popup failed: ' + String(error && error.message || error);
    }
  }

  /* A MAP button on Pipeline News' INTERCONNECTORS tab sends
       ?interconnector=INTNED&technology=interconnector&anchor=midpoint|gb_converter
     with the anchor's coordinate. The project arrival lane hands this over.
     The link is looked up by BMRS code; the span is framed after the shell's
     own flyTo has settled (racing it is a fight the shell wins); the card
     opens on the drawn node with each converter measured within the per-end
     search budget. A link whose far converter is not yet held has no line to
     frame: the map stays at the GB end the link opened on, and the state says
     why rather than measuring anything from anywhere else. */
  function arrive(map, geojson) {
    let q;
    try { q = new URLSearchParams(window.location.search); } catch (_) {
      if (isInterconnectorArrival()) answer(false, 'QUERY_UNREADABLE', null);
      return;
    }
    const code = String(q.get('interconnector') || '').trim().toUpperCase();
    if (!code) {
      /* technology=interconnector with no BMRS code was total silence: the
         project lane stands down at its guard, this lane stands down here, and
         nothing on the page had asked a question or refused one. */
      if (String(q.get('technology') || '') === 'interconnector') {
        answer(false, 'NO_BMRS_CODE', null);
      }
      return;
    }
    state.arrival = { requested: code, anchor: q.get('anchor') || null, status: 'PENDING', card: null };
    showLayers(map);
    const link = state.links.find((l) => String(l.bmrs || '').toUpperCase() === code
      || String(l.link || '').toUpperCase() === code);
    const line = (geojson.features || []).find((f) => f.properties
      && f.properties.feature_role === 'link-line'
      && String(f.properties.bmrs_code || '').toUpperCase() === code);
    const node = (geojson.features || []).find((f) => f.properties
      && f.properties.feature_role === 'midpoint'
      && String(f.properties.bmrs_code || '').toUpperCase() === code);
    if (!link || !line) {
      /* No far converter held, so no line and no span. The link still names
         a GB converter and sends its coordinate: fly there, measure that one
         end against the payload when it arrives, and say plainly that the
         other end is not located. Measured on v9.146's first cut: without
         this the map stayed at zoom 4.2 while the state claimed it had moved. */
      state.arrival.status = 'NOT_DRAWABLE';
      state.arrival.statement = 'No drawable line is held for ' + code
        + ': its far converter is not located yet. Only the GB converter is shown and measured.';
      const lon = Number(q.get('longitude')), lat = Number(q.get('latitude')), z = Number(q.get('zoom'));
      const usable = Number.isFinite(lon) && Number.isFinite(lat) && Math.abs(lon) <= 180 && Math.abs(lat) <= 90
        && !(Math.abs(lon) < 1e-9 && Math.abs(lat) < 1e-9);
      if (!usable) {
        state.arrival.framed = false;
        answer(false, 'FAR_END_NOT_LOCATED_AND_NO_ANCHOR_COORDINATE', 'far_end_converter');
        return;
      }
      const name = q.get('project') || code;
      const go = () => {
        try {
          map.flyTo({ center: [lon, lat], zoom: Number.isFinite(z) ? Math.min(14, Math.max(5, z)) : 10, duration: 600 });
          state.arrival.framed = 'gb_converter';
        } catch (error) { state.arrival.framed = false; state.arrival.frame_error = String(error && error.message || error); }
        const measureGb = (attempt) => {
          const subs = (window.__GRIDATLAS_NEON_LINKS__ || {}).substations || window.__GRIDATLAS_SUBSTATIONS__ || [];
          if (!subs.length && attempt < 120) { setTimeout(() => measureGb(attempt + 1), 500); return; }
          const budget = searchKm();
          const gb = measureEnd('gb', lon, lat, budget, subs);
          state.arrival.ends = [gb, null];
          state.arrival.search_km = budget;
          state.arrival.substations_seen = subs.length;
          state.arrival.waited_ms = attempt * 500;
          /* The GB end has been measured against the published payload. That
             is the answer, and it stands whether or not a popup can be drawn;
             the far converter is a fallback that is named, not a failure. */
          answer(true, 'FAR_END_NOT_LOCATED', 'far_end_converter');
          const gl = window.maplibregl;
          if (!gl || !gl.Popup) { state.arrival.card = 'no maplibregl.Popup'; return; }
          const html = '<div style="font-family:monospace;background:#000;color:#fff;padding:8px 10px;min-width:280px;max-width:420px;font-size:12px;line-height:1.5">'
            + '<div style="font-size:14px"><b>' + esc(name) + '</b> · ' + esc(code) + ' · interconnector</div>'
            + '<div><b>GB end</b> ' + esc(endLine(gb, budget)) + '</div>'
            + '<div style="color:#ffae00">Far converter not located yet: no line is drawn and nothing is measured at that end.</div>'
            + '<div style="color:#a6adbb">search budget ' + esc(budget) + ' km (' + esc(state.search_source) + '); widen it in the radius box</div>'
            + '</div>';
          try {
            state.arrival.popup = new gl.Popup({ closeOnClick: false, maxWidth: '440px' }).setLngLat([lon, lat]).setHTML(html).addTo(map);
            state.arrival.card = 'OPEN';
          } catch (error) { state.arrival.card = 'popup failed: ' + String(error && error.message || error); }
        };
        measureGb(0);
      };
      setTimeout(go, 1200);
    ...
    const at = node ? node.geometry.coordinates : [(a[0] + b[0]) / 2, (a[1] + b[1]) / 2];
    let done = false;
    /* The substation payload is 1.2 MB and arrives after the map does; the
       module used to measure at boot against an empty list, so the GB end
       always read "no payload" - measured on v9.143. Wait for the lane's
       published list (up to 60 s), measure both ends, then frame and open. */
    const measureWhenReady = (attempt) => {
      const subs = (window.__GRIDATLAS_NEON_LINKS__ || {}).substations || window.__GRIDATLAS_SUBSTATIONS__ || [];
      if (!subs.length && attempt < 120) { setTimeout(() => measureWhenReady(attempt + 1), 500); return; }
      const budget = searchKm();
      link.ends = [measureEnd('gb', a[0], a[1], budget, subs), measureEnd('far', b[0], b[1], budget, [])];
      link.label = label({ gb_converter: link.gb_converter, far_converter: link.far_converter, link: link.link, straight_line_km: link.span_km }, budget);
      state.arrival.ends = link.ends;
      state.arrival.search_km = budget;
      state.arrival.substations_seen = subs.length;
      state.arrival.waited_ms = attempt * 500;
      try { map.once('idle', settle); } catch (_) { /* shimmed map */ }
      setTimeout(settle, 1500);
    };
    const settle = () => {
      if (done) return;
      done = true;
      try { map.off('idle', settle); } catch (_) { /* shimmed map */ }
      try {
        map.fitBounds([[Math.min(a[0], b[0]), Math.min(a[1], b[1])], [Math.max(a[0], b[0]), Math.max(a[1], b[1])]],
          { padding: 90, duration: 600, maxZoom: 9 });
        state.arrival.framed = true;
      } catch (error) {
        state.arrival.framed = false;
        state.arrival.frame_error = String(error && error.message || error);
      }
      openCard(map, link, at);
      /* Both converters measured, the span framed and the card open. The far
         end is measured against an empty substation set by construction - the
         contract is absent coverage, never absence of connection - so that is
         named as the fallback rather than passed over. */
      const gbEnd = Array.isArray(link.ends) ? link.ends[0] : null;
      answer(true, gbEnd && gbEnd.coverage === 'NONE' ? 'GB_SUBSTATION_PAYLOAD_ABSENT' : null,
        'far_end_substation_coverage');
    };
    measureWhenReady(0);
    state.arrival.status = 'RESOLVED';
    state.arrival.link = link.link;
    state.arrival.bmrs = link.bmrs;
    state.arrival.span_km = link.span_km;
    state.arrival.search_km = state.search_km;
    state.arrival.ends = link.ends;
    state.arrival.at = at;
  }
  ...
  async function boot(map) {
    let geojson;
    try {
      const absolute = new URL(DATA_URL, document.baseURI).href;
      const body = await new Promise((resolve, reject) => {
        ...
        xhr.onerror = () => reject(new Error("transport failed"));
        xhr.send();
      });
      geojson = JSON.parse(body);
    } catch (error) {
      state.error = 'interconnector data not served: ' + String(error && error.message || error);
      /* arrive() is never reached, so without this an interconnector arrival on
         a page whose data file did not serve is completely silent: the project
         lane stood down at its guard and this lane never started. */
      if (isInterconnectorArrival()) answer(false, 'INTERCONNECTOR_DATA_NOT_SERVED', null);
      return;
    }
    ...
    state.links = (geojson.features || [])
      ...
      .map((f) => {
        ...
        return {
          link: p.link, bmrs: p.bmrs_code, gb_converter: p.gb_converter, far_converter: p.far_converter,
    ...
    state.search_km = km;
    paint(map, geojson);
    try { arrive(map, geojson); }
    catch (error) {
      state.error = 'interconnector arrival threw: ' + String(error && error.message || error);
      if (isInterconnectorArrival()) answer(false, 'INTERCONNECTOR_ARRIVAL_THREW', null);
    }
  }

  function ready(map) {
    if (!map) return;
    if (map.loaded && map.loaded()) { boot(map); return; }
    let started = false;
    const go = () => { if (!started) { started = true; boot(map); } };
    map.once('load', go);
    map.once('idle', go);
    const settle = setInterval(() => {
      if (map.loaded && map.loaded()) { clearInterval(settle); go(); }
    }, 500);
    setTimeout(() => clearInterval(settle), 60000);
  }
  ...
  const timer = setInterval(() => {
    ...
    try { ready(map); }
      ...
      if (isInterconnectorArrival()) answer(false, 'INTERCONNECTOR_MODULE_THREW', null);
    }
  }, 400);
  setTimeout(() => {
    clearInterval(timer);
    /* Ninety seconds and no map ever appeared on the window. The module never
       got as far as loading, so nothing below it can speak for it. */
    if (!state.loaded && isInterconnectorArrival()) answer(false, 'NO_MAP_WITHIN_90S', null);
  }, 90000);
})();
