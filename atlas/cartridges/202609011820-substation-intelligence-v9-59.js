/**
 * GridAtlas substation intelligence, generation 202609011820 (UTC), composition v9.59.
 * Slot: replace-script for ventus-corev8engine.js.
 *
 * PART 1 is the V8 engine, carried forward VERBATIM - every byte of the
 * shell's own engine, unchanged. The composer replaces a script TAG, so a
 * cartridge may only claim a script the shell actually loads; the shell
 * loads four and the engine is the one still unclaimed. v9.57 claimed a
 * file that merely sat beside them in the release directory, the composer
 * refused it, and the map went dark until v9.58 restored service. The
 * lesson is in this generation's proof: every composed cartridge's slot is
 * now checked against the shell's index.html before anything ships.
 *
 * PART 2 is the substation intelligence: what the system operator
 * publishes about a substation, which no OpenStreetMap payload can know.
 * It reads Ventusltd/data-grid-gb, built from NESO's Electricity Ten Year
 * Statement appendices - circuits and their seasonal ratings, transformers,
 * reactive plant, eight separately named fault current metrics across the
 * published demand scenarios, and the changes already planned to 2033/34.
 *
 * It publishes and measures. It renders nothing and decorates nothing: the
 * sandbox owns the card, this owns the computation. It grades nothing - a
 * rating is a rating and a fault level is a fault level; queue position,
 * committed connections, consent and commercial terms decide connection,
 * and no published appendix contains them. It fails closed on a schema it
 * was not written against, and it revalidates rather than pinning the
