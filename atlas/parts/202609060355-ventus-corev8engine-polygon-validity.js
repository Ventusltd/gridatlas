...
window.initVentusMap = function({ config, center, zoom }) {
    ...
    function _zoneDrawUpdateLayers(dragOnly) {
        if(!dragOnly)_zoneDrawRefreshEditor();
        if (!dragOnly) {
        ...
        if (!dragOnly && zoneDrawPoints.length >= 3) _zoneDrawStorageStatus(window.__GRIDATLAS_MODULES__.polygonDraft.saveOutline(zoneDrawPoints));
