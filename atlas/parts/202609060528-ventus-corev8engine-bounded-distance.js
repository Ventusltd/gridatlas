...
window.initVentusMap = function({ config, center, zoom }) {
    ...
    /* How much clear screen space an add-a-point dot needs beyond a corner's
       own grab radius before it is allowed to exist on that edge. */
    ...
    function haversine(lon1, lat1, lon2, lat2) {
        ...
        // Floating-point rounding can put antipodal pairs just above one.
    ...
    function _zoneDrawRefreshEditor() {
        ...
        const remove=document.getElementById('btn-zonedraw-remove');if(remove)remove.disabled=zoneDrawLocked||zoneDrawPoints.length<=3;
    }
    function _zoneDrawUpdateLayers(dragOnly, renderOnly = false) {
        ...
        const validity=window.__GRIDATLAS_MODULES__.polygonFiles.analyzeOutline(zoneDrawPoints);
        if (exportButton) {exportButton.disabled=!validity.valid;exportButton.title=validity.valid?'Save a closed polygon':validity.message;}
        ...
        map.getSource('src-zonedraw-fill').setData({ type: 'FeatureCollection', features: validity.valid ? [{ type: 'Feature', geometry: { type: 'Polygon', coordinates: [ring] } }] : [] });
        ...
        if (dragOnly) {
            ...
            zoneDrawPoints.forEach((c, i) => {
                /* Same rule as the hit test, so a dot is never drawn where it
                   cannot be pressed and never pressable where it is not drawn. */
            ...
            measurementDock.show( `
                <div style="font-family:monospace;background:#000;padding:10px 12px;border:1px solid #ff6600;border-radius:4px;min-width:230px;">
                    <div style="display:flex;justify-content:space-between;align-items:center;margin-bottom:8px;">
                        ...
                        <button type="button" data-zonedraw-toggle aria-label="Collapse polygon measurements" aria-expanded="true" onclick="window._zdCollapse&&window._zdCollapse()" style="color:#bbb;background:#000;border:1px solid #777;border-radius:3px;font-size:12px;cursor:pointer;padding:0 8px;" title="Collapse">▴ hide</button>
                    </div>
                    ...
                    <div style="color:#aaa;font-size:11px;margin-top:8px;line-height:1.4;">
                        ...
                        • <b>Undo</b> restores the previous edit<br>
                        • <b>Reset polygon</b> to start a new zone
                    </div>
                </div>`);
            window._zdCollapse = () => { _zoneDrawCollapsed = true; _zoneDrawShowPopup(); document.querySelector('[data-zonedraw-toggle]')?.focus(); };
        }
    }
    ...
    /* An edge is only long enough to carry add-a-point dots when its ends are
       far enough apart on screen. A 24-gon zoomed out puts three dots inside
       the corner's own hit radius, so the dots crowd out the corners and the
       polygon becomes uneditable. One rule, used by both the hit test and the
       drawing, so what you can press is always what you can see. */
    ...
    function _zoneDrawOnClick(e) {
        ...
        if (edgeHit) {
            if(zoneDrawPoints.length>=4096){_zoneDrawStorageStatus({message:'This outline already has 4096 vertices. Remove an unneeded corner before adding another.'});return;}
            zoneDrawPoints.splice(edgeHit.insertIdx, 0, [edgeHit.dot[0], edgeHit.dot[1]]);
            _zoneDrawUpdateLayers(false); _zoneDrawShowPopup();
            return;
        }

        // A completed outline belongs to the user. Background clicks cannot replace it.
        // Reset polygon is the explicit transition back to placing a fresh circle.
        return;
    }
    ...
    function _zoneDrawOnMouseDown(e) {
        ...
        /* A CORNER ALWAYS WINS A PRESS.
           This used to test the edge dot first and return, while the hover
           handler tested the vertex first. So the cursor promised a grab and
           the press silently refused, which is what "cannot select the elastic
           bands" was. Measured on the live map at Cleve Hill: at the drawn zoom
           the nearest midpoint sits 57.7 px from a corner and all 24 corners
           drag; two zoom steps out it sits 14.4 px away and none of them do.
           Hover and press now ask the same question in the same order. */
    ...
    function buildDOM() {
        ...
        const summary=document.createElement('summary');summary.textContent='Edit or remove vertices';summary.style.cssText='min-height:44px;cursor:pointer';zoneEditor.append(summary);
        ...
        const zoneRemove=document.createElement('button');zoneRemove.id='btn-zonedraw-remove';zoneRemove.type='button';zoneRemove.textContent='Remove selected vertex';zoneRemove.disabled=true;zoneRemove.style.cssText=zoneExport.style.cssText;zoneEditor.append(zoneRemove);
        zoneRemove.title='Keep at least three vertices. Undo restores a removed vertex.';
        zoneRemove.addEventListener('click',()=>{
            if(zoneDrawLocked||zoneDrawPoints.length<=3)return;
            const i=Number(document.getElementById('zonedraw-vertex').value);
            if(!Number.isInteger(i)||!zoneDrawPoints[i])return;
            zoneDrawPoints.splice(i,1);_zoneDrawUpdateLayers(false);_zoneDrawShowPopup();
            _zoneDrawStorageStatus({message:'Removed vertex '+(i+1)+'. Undo restores the exact outline.'});
        });
    ...
    */
    ...
    map.on('load', () => {
        ...
        map.addLayer({ id: 'l-zonedraw-points', type: 'circle', source: 'src-zonedraw-points', paint: {
            /* Corners read as handles: bigger, brighter, and ringed in white so
               they stand off the imagery at any zoom. Add-a-point dots stay
               deliberately quieter than the corners they sit between. */
        ...
        /* A FINGER IS ALSO A POINTER.
           Before this, zone editing had no touch path at all: dragging a corner
           was bound to mousedown/mousemove/mouseup only, so on a phone or a
           tablet the corners could be seen and never moved. A browser does
           synthesise a click after a tap, which is why placing a zone worked on
           a phone while shaping one did not - the failure looked like a fiddly
           control rather than an absent one.

           Touch coordinates are page-relative, so they are converted through the
           canvas rectangle rather than read from offsetX/offsetY, which a touch
           event does not carry. preventDefault is called only once a corner has
           actually been grabbed, so an ordinary one-finger pan over the map is
           untouched. */
        /* Which add-a-point dots fit on an edge depends on how long that edge
           is ON SCREEN, so it changes with the zoom. Without this the dots are
           computed once when the zone is drawn and then go stale: zoom out and
           the drawing still shows dots the hit test has already suppressed, so
           what you see stops being what you can press. Redrawn on zoom, and only
           when a zone exists. */
        map.on('zoomend', () => {
