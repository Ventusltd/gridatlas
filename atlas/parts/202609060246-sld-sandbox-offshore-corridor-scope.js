...
(() => {
  ...
  // Highway-calibrated cable factors do not describe an offshore export route.
  // Keep the measured straight line; use the existing selection policy for scope.
  const OFFSHORE_CORRIDOR_NOTE = 'Offshore export route unassessed: the highway-calibrated '
    + 'corridor factor does not apply. Offshore substation, export cable, landfall and '
    + 'onshore route require separate assessment.';
  function corridorBeside(km) {
    if (currentPolicy?.offshore) {
      return ` <span class="neon-caveat">${OFFSHORE_CORRIDOR_NOTE}</span>`;
    }
    const module = (() => {
  ...
  function openCorridorSheet(sheet) {
    ...
    const body = rows.map(row => {
      const estimate = !currentPolicy?.offshore && module ? module.forCable(row.km) : null;
    ...
    const basis = !currentPolicy?.offshore && module ? module.basis : null;
    sheet.innerHTML = `<h4>Route corridors</h4>${body}`
      + (basis
        ...
        : currentPolicy?.offshore
          ? `<div class="r c">${OFFSHORE_CORRIDOR_NOTE}</div>`
