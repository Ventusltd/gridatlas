...
(() => {
  ...
  function close() {
    if (!state) return;
    const {map, host, rail, control, marker, style, restoreLayers} = state;
    state = null;
    restoreLayers();
  ...
  function open(map, controlId) {
    ...
    let layersControl=null,layersMarker=null;
    const dockLayers=()=>{
      if(layersControl)return;
      const button=document.getElementById('gridatlas-dash-toggle');if(!button)return;
      layersControl=button;layersMarker=document.createComment('layers launcher home');
      button.before(layersMarker);controls.prepend(button);observer.disconnect();
    };
    const observer=new MutationObserver(dockLayers);
    observer.observe(document.body,{childList:true,subtree:true});dockLayers();
    const restoreLayers=()=>{observer.disconnect();if(layersMarker&&layersControl)layersMarker.replaceWith(layersControl);};
...
#gridatlas-measurement-dock{position:absolute;left:0;top:64px;bottom:0;width:280px;box-sizing:border-box;overflow:auto;overscroll-behavior:contain;background:#080b10;border-right:1px solid #58646d;color:#fff;z-index:4100;padding:10px;font:12px/1.4 ui-monospace,monospace;display:flex;flex-direction:column;gap:8px}
#gridatlas-measurement-dock .measurement-dock-controls,#gridatlas-measurement-dock .measurement-dock-values{flex-shrink:0}
#gridatlas-measurement-dock .measurement-dock-values{order:-1}
...
#gridatlas-measurement-dock button,#gridatlas-measurement-dock input{min-height:44px}
#gridatlas-measurement-dock :focus-visible{outline:2px solid #fff;outline-offset:2px}
#gridatlas-measurement-dock #gridatlas-dash-toggle{position:static!important;display:block;width:100%;margin:0 0 8px;box-sizing:border-box}
body[data-measurement-dock] #testcode-compute-receipt,body[data-measurement-dock] .testcode-identity{visibility:hidden!important}
body[data-measurement-dock] #codex-tool-layers{visibility:hidden!important;pointer-events:none!important}
@media(max-width:700px){
 body[data-measurement-dock] .scada-wrapper{left:12px!important;right:12px!important;width:auto!important;max-height:calc(100dvh - min(240px,38dvh) - 136px)!important;box-sizing:border-box}
 body[data-measurement-dock] .scada-wrapper[data-gridatlas-collapsed="1"]{max-height:0!important}
 ...
 #gridatlas-measurement-dock{top:auto;right:0;width:100%;height:min(240px,38dvh);border-right:0;border-top:1px solid #58646d;padding:8px 12px;display:grid;grid-template-columns:minmax(110px,0.8fr) minmax(180px,1.4fr);gap:8px;z-index:4100}
 #gridatlas-measurement-dock .measurement-dock-controls{grid-column:1;grid-row:1}
 #gridatlas-measurement-dock .measurement-dock-values{grid-column:2;grid-row:1}
...
@media(max-width:350px){#gridatlas-measurement-dock{grid-template-columns:1fr}
 #gridatlas-measurement-dock .measurement-dock-values{grid-column:1;grid-row:1}
 #gridatlas-measurement-dock .measurement-dock-controls{grid-column:1;grid-row:2}}
`;
    document.head.append(style);
    host.append(rail);
    host.setAttribute('data-measurement-dock', controlId);
    document.body.setAttribute('data-measurement-dock', controlId);
    state = {map, host, rail, control, marker, style, values, restoreLayers};
    map.resize();
  }
  function show(html) {
  ...
  function exportOutline(points, measurements) {
    ...
    const origin=ring[0];
    const winding = ring.reduce((sum,point,i) => {const next=ring[(i+1)%ring.length];return sum+(point[0]-origin[0])*(next[1]-origin[1])-(next[0]-origin[0])*(point[1]-origin[1]);},0);
