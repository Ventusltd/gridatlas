...
const site = path.resolve(process.argv[3] || "build-a/202608291239-atlas-v9");
...
const releaseId = "202608291239-atlas-v9";
...
class AtlasMap {
  ...
  addLayer(layer) {
    if (layer.type === "symbol" && layer.layout?.["text-field"] && !this.options.style?.glyphs) throw new Error("symbol text layer requires glyphs: " + layer.id);
    this.layers.set(layer.id, layer); globalThis.__atlasAudit.layers.push(layer.id);
  }
...
try {
  ...
  const clusterProof = await page.evaluate(() => {
    const audit = globalThis.__atlasAudit;
    const map = audit.map;
    const clusterHandler = (map.handlers.click || []).find(entry => entry.layer === "repd-clusters")?.handler;
    if (clusterHandler) clusterHandler({ features: [{ properties: { cluster_id: 7 }, geometry: { coordinates: [-1, 52] } }] });
    return {
      layers: ["repd-clusters", "repd-points", "repd-selected"].filter(id => Boolean(map.getLayer(id))),
      countLayer: Boolean(map.getLayer("repd-cluster-count")),
      handler: Boolean(clusterHandler),
      expansion: audit.easeTo?.at(-1) || null
    };
  });
  assert.deepEqual(clusterProof.layers, ["repd-clusters", "repd-points", "repd-selected"], "REPD cluster/point/selection layers missing");
  assert.equal(clusterProof.countLayer, false, "glyph-dependent count layer rendered");
  assert.equal(clusterProof.handler, true, "cluster expansion click handler missing");
  assert.deepEqual(clusterProof.expansion, { center: [-1, 52], zoom: 10 }, "cluster click did not expand with easeTo");
  ...
  await page.screenshot({ path: path.join(evidence, "202608291239-browser-desktop.png"), fullPage: true });
  await desktop.close();
  ...
  await mobilePage.screenshot({ path: path.join(evidence, "202608291239-browser-mobile.png"), fullPage: true });
  await mobile.close();
  ...
  const report = {
    ...
    glyphless_style_symbol_guard: true,
    repd_cluster_expand_click: true,
    repd_cluster_layers: ["repd-clusters", "repd-points", "repd-selected"],
  ...
  fs.writeFileSync(path.join(evidence, "202608291239-browser-audit.json"), JSON.stringify(report, null, 2) + "\n");
