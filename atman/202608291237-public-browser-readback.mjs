...
const liveUrl = new URL(process.argv[2] || "https://ventusltd.github.io/gridatlas/202608291237-atlas-v9/");
...
assert.equal(liveUrl.pathname, "/gridatlas/202608291237-atlas-v9/");
...
try {
  ...
  await page.screenshot({ path: path.join(evidence, "202608291237-public-live-duckdb.png"), fullPage: true });
  const report = {
  ...
  fs.writeFileSync(path.join(evidence, "202608291237-public-browser-readback.json"), JSON.stringify(report, null, 2) + "\n");
  const claim = {
  ...
  fs.writeFileSync(path.join(evidence, "202608291237-public-browser-claim.json"), JSON.stringify(claim, null, 2) + "\n");
