...
import './202609040400-sld-styles.proof.mjs';
...
/* Styles now live in an earlier sibling cartridge while their DOM lifecycle
   remains here. Assertions about CSS inspect the composed bytes; assertions
...
check('the fresh composition does not carry the obsolete offshore no-links claim',
  CURRENT.cartridges.find(({ id }) => id === 'sld-sandbox')?.capabilities
    ?.includes('offshore-measures-with-route-caveat') === true
  && !JSON.stringify(CURRENT).includes('offshore-opens-a-card-and-withholds-the-measurement')
  && !JSON.stringify(CURRENT).includes('opens a card, draws no links')
  && !JSON.stringify(COMPOSITION).includes('offshore-opens-a-card-and-withholds-the-measurement')
  && !JSON.stringify(COMPOSITION).includes('opens a card, draws no links'));
...
check('the per-row hint reads as a hint',
  /cursor:help/.test(uiSource) && kvSrc.includes("kv + ' kV: ' + context"));
...
check('it is sized against the viewport, not a desktop column',
  /max-width:min\(88vw,260px\)/.test(uiSource)
  && /max-height:min\(52vh,340px\)/.test(uiSource));
...
check('it is sized against the viewport, not a desktop column',
  /max-width:min\(92vw,420px\)/.test(uiSource));
check('the pulse honours a reduced-motion preference',
  /@media \(prefers-reduced-motion:no-preference\)/.test(uiSource));
...
check('the unstated case is called out in red',
  /class="sld-danger"/.test(src) && /#ff5d5d/.test(uiSource));
...
check('with a minimise and a close big enough to hit',
  ...
  && /gridatlas-card-bar button\{[\s\S]{0,220}min-width:44px;height:44px/.test(uiSource));
check('dragging frees the card from its anchor',
  /gridatlas-free/.test(code) && /position:fixed !important/.test(uiSource));
check('a freed card does not snap back on pan',
  /transform:none !important/.test(uiSource));
...
check('a minimised card is styled to read as restorable',
  /gridatlas-min .gridatlas-card-bar button.min/.test(uiSource)
  && /box-shadow:0 0 14px/.test(uiSource));

check('the card is bounded to the map and scrolls',
  /max-height:var\(--gridatlas-card-max/.test(uiSource)
  && /overflow-y:auto !important/.test(uiSource));
check('the cap comes from the real map height, not the viewport',
  /function boundCardToMap/.test(code) && /getContainer\(\)/.test(code));
check('the bar stays put while the card scrolls', /position:sticky/.test(uiSource));
...
check('the bar cannot stretch', /flex:0 0 auto/.test(uiSource));
...
check('a phone gets a sheet docked to the bottom edge, not an anchored card',
  ...
  && /bottom:0 !important/.test(uiSource));
check('it is full width, so nothing is clipped off the left edge',
  /width:100vw !important;max-width:100vw !important/.test(uiSource));
...
check('the floating controls are lifted clear of the sheet, not buried under it',
  /html\.gridatlas-sheet-open \.map-controls\{/.test(uiSource)
  && /bottom:calc\(var\(--gridatlas-sheet-h/.test(uiSource)
  && !/html\.gridatlas-sheet-open \.map-controls\{display:none/.test(uiSource));
check('the lift reads the same variable the sheet is sized from',
  /--gridatlas-sheet-h', height \+ 'px'/.test(code)
  && (uiSource.match(/var\(--gridatlas-sheet-h/g) || []).length >= 3);
...
check('the layout panel is bounded by top and bottom rather than an overflowing height',
  /#\$\{PANEL_ID\}\{position:absolute;right:14px;top:112px;bottom:14px/.test(uiSource)
  && !/max-height:calc\(100% - 28px\)/.test(uiSource));
check('both panel control pairs are 44px phone targets',
  /gridatlas-card-bar button\{[\s\S]{0,220}min-width:44px;height:44px/.test(uiSource)
  && /sld-min,#\$\{PANEL_ID\} \.sld-close\{[\s\S]{0,240}min-width:44px;height:44px/.test(uiSource));
check('both drag handles suppress browser touch scrolling during capture',
  /gridatlas-card-bar\{[\s\S]{0,260}touch-action:none/.test(uiSource)
  && /h4\.sld-drag\{[^}]*touch-action:none/.test(uiSource));
check('the 844 by 390 control stack scrolls inside the short viewport',
  /@media \(max-height:600px\)\{[\s\S]{0,260}\.map-controls\{[^}]*max-height:[^}]*overflow-y:auto/.test(uiSource));
check('the 844 by 390 search result list is viewport bounded',
  /@media \(max-height:600px\)\{[\s\S]{0,420}\.search-results\{max-height:calc\(100dvh - 140px\)/.test(uiSource));
check('coarse pointers enlarge shell and sandbox controls',
  /@media \(pointer:coarse\)\{[\s\S]*?\.map-ctrl-btn,\.search-btn\{min-height:44px\}/.test(uiSource)
  && /sld-tabs button,#\$\{PANEL_ID\} input,#\$\{PANEL_ID\} select,[\s\S]{0,100}sld-finance summary\{min-height:44px\}/.test(uiSource));
...
check('the six shell tool buttons collapse behind one chip',
  /\.map-controls\.gm-tools-collapsed > \.map-ctrl-btn\{display:none\}/.test(uiSource)
...
const gazetteerEntry = CURRENT.cartridges.find(({ id }) => id === 'uk-gazetteer-flyto');
const gazSource = await readFile(join(REPO, 'atlas',
  gazetteerEntry.path.replace(/^\.\//, '')), 'utf8');
...
check('only a ref-only link waits for identity before it can measure',
  /else if \(receiverPlan\.route === 'WAIT_FOR_REGISTER'\) \{[\s\S]{0,900}const owner = await waitForResolvedIdentity\(\);/
    .test(cartridgeSource));
...
check('a true identity-loader failure retries owner and measurement on one fresh epoch',
  /The active-register identity check failed:/.test(cartridgeSource)
  && /async function retryIdentityOwnerThenArrival\(\)/.test(cartridgeSource)
  && /const retryEpoch = claimPendingArrival\(window\.location\.search\)/.test(cartridgeSource)
  && /await owner\.retry_exact_deep_link\(retryEpoch\)/.test(cartridgeSource)
  && /await rerunDeepLink\(retryEpoch\)/.test(cartridgeSource)
  && /retryArrival = retryIdentityOwnerThenArrival/.test(cartridgeSource));
check('an expected ref-only absence is explicit and makes no official-location claim',
  /is not in the active-register/.test(cartridgeSource)
  && /No official status or location is inferred/.test(cartridgeSource)
  && /status: 'NOT_IN_ACTIVE_REGISTER'/.test(cartridgeSource));
...
check('the deep link is a named function so retry re-runs it on the shared epoch',
  /async function runDeepLink\(expectedArrivalEpoch = null\) \{/.test(cartridgeSource)
  && /rerunDeepLink = runDeepLink;/.test(cartridgeSource)
  && /void runDeepLink\(\);/.test(cartridgeSource));
...
/* In the exact Pipeline 0144 corpus, 2,430 of 8,743 unique clickable REPD
   refs are absent from the active snapshot while 6,313 match it. Before
   this repair, absent arrivals drew links and no card ever opened. */
...
check('the fallback card states its provenance',
  /Card and point built from the arrival link\./.test(cartridgeSource));
...
check('the golden browser field records this generation\'s five mobile arrival cases',
  manifest.acceptance.golden_browser_verification
    === 'PASS_393x852_FIVE_ARRIVAL_CASES');
check('the stylesheet hoist browser receipt records the exact independent comparison',
  JSON.stringify(manifest.acceptance.stylesheet_hoist_browser_verification)
    === JSON.stringify({
      before_generation: '202609040337',
      after_generation: '202609040403',
      clean_profile_runs: 4,
      viewports: ['393x852', '1400x900'],
      seven_css_values: 'EXACT_SHA256_PARITY',
      computed_styles: 'EXACT_PARITY',
      fullscreen_element: 'map-container',
      menus_in_fullscreen: 6,
      mobile_tray: 'COARSE_ONLY',
      page_errors: 0,
      console_errors: 0,
      result: 'PASS'
    }));
...
console.log('\nrun against a DOM, not a regular expression\n');
{
  ...
  const box = {
    window: { ...windowStub, __GRIDATLAS_MODULES__: {} }, document: behaviourDoc, console,
   ...
   PIPELINENEWS_REPO only selects an equivalent checkout for local replay. */
{
  ...
  for (const candidate of candidates) {
    try {
      spineText = await readFile(join(candidate, 'releases', '202609040144-pipelinenews',
        'data', '202608270055-8ab1807551bc-v8-fast-projects.json'), 'utf8');
      widerText = await readFile(join(candidate, 'releases', '202609040144-pipelinenews',
        'data', '202609040044-wider-fleet.json'), 'utf8');
  ...
  if (spineText && widerText) {
    check('the two corpus files are the exact reviewed bytes',
      ...
      && createHash('sha256').update(widerText).digest('hex')
        === '29966f9b5573295e8c7c3793b1950a336c50fbe7570ba63b83f237bb57271efe');
    ...
    check('the sender inputs contain 7,652 valid spine plus 1,101 served wider rows',
      spine.length === 7652 && wider.length === 1101
      && links.length === spine.length + wider.length && links.length === 8753,
      `${spine.length} + ${wider.length} = ${links.length}`);
    check('direct referenced rows remain distinguishable from coordinate-only rows',
      refCount === 8739, String(refCount));
