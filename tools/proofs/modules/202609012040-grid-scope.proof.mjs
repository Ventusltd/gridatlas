...
const scopeSource = await readFile(
  join(REPO, 'atlas', 'modules', '202609012040-grid-scope.js'), 'utf8');
...
for (const name of ['202609011950-geodesy.js', '202609012040-grid-scope.js']) {
...
check('the module registers',
  gridScope?.schema === 'gridatlas.module.grid-scope.v2');
...
/* Codex stop-ship 202609012025, on the committed v1 module: it walked the
   class list and returned the first class the value exceeded, so it
   labelled 750 kV as 400, 110 kV as 66 and 50 kV as 33. These are its
   exact counterexamples. */
check('750 kV is not labelled 400 kV', gridScope.classOf(750) === null);
check('110 kV is not labelled 66 kV', gridScope.classOf(110) === null);
check('50 kV is not labelled 33 kV', gridScope.classOf(50) === null);
check('32 kV, just below a class, is not rounded up',
...
check('an off-class site is counted as unclassified, not folded into a class', (() => {
  const odd = gridScope.scope(HOME, [
    { name: 'Odd 110', at: at(0.01), kv: [110] },
    { name: 'Real 400', at: at(0.012), kv: [400] }
  ], {});
  const band = odd.bands[0];
  return band.substations === 2 && band.unclassified_voltage === 1
    && band.by_class_kv['400'] === 1 && !band.by_class_kv['66']
    && band.unclassified_kv.includes(110) && band.highest_class_kv === 400;
})());
