 ...
 *
 * WHAT THIS ADDS OVER THE FIRST PROOF
 * -----------------------------------
 * The first proof passed while the production caller was broken, because
 * it tested a case production never takes: it chose a CONNECTED distant
 * slack, and the live caller chose the first lexicographic bus. Codex
 * found that on review. So the cases below include the production path
 * itself, run on the real product, and the two shapes that were wrong:
 * a cross-component transfer, and parallel circuits sharing a reactance.
 *
...
const FLOW = 'atlas/modules/202609020015-injection-response.js';
...
/* CASE 9 - REGRESSION, Codex 202609020030 finding 2. Two genuine parallel
   circuits that publish the SAME reactance are two circuits. Collapsing
   them into one understated the network on 45 published rows. */
{
  const g = graphOf(['A', 'B'], [X('A', 'B', 100), X('A', 'B', 100)]);
  const m = flow.assemble(g, ['A', 'B'], { voltageKv: 400, includeTransformers: false });
  check('two parallel circuits of EQUAL reactance are two edges, not one',
    m.counts.branches === 2, JSON.stringify(m.counts));
  const r = flow.respond(m, { atNode: 'A', slackNode: 'B', mw: 200, minimumShare: 0 });
  check('and they share the transfer equally, half each',
    r.branches.length === 2
    && close(r.branches[0].share_of_injection, 0.5)
    && close(r.branches[1].share_of_injection, 0.5));
  check('each carries 100 MW of a 200 MW transfer, not 200',
    r.branches.every(b => close(b.flow_mw, 100, 1e-9)));
}

/* CASE 10 - REGRESSION, finding 1. A transfer between two components is
   refused BEFORE the solver is asked, because the solver answers anyway. */
{
  const g = graphOf(['A', 'B', 'C', 'D'],
    [X('A', 'B', 50), X('C', 'D', 50)]);
  const m = flow.assemble(g, ['A', 'B', 'C', 'D'], { voltageKv: 400, includeTransformers: false });
  check('the model counts its connected components', m.counts.components === 2,
    String(m.counts.components));
  const r = flow.respond(m, { atNode: 'A', slackNode: 'D', mw: 100, minimumShare: 0 });
  check('a cross-component transfer is refused, not solved',
    r.publishable === false && r.branches.length === 0);
  check('and the refusal says why, naming components',
    /different connected components/.test(r.reason));
  check('this is the exact shape that returned passes:true before',
    r.publishable === false);
}

/* CASE 11 - acceptance is global. A solve that has not converged must not
   be publishable even if the injection bus happens to balance. */
{
  const g = graphOf(['A', 'B', 'C'], [X('A', 'B', 50), X('B', 'C', 50)]);
  const m = flow.assemble(g, ['A', 'B', 'C'], { voltageKv: 400, includeTransformers: false });
  const r = flow.respond(m, { atNode: 'A', slackNode: 'C', mw: 100, minimumShare: 0 });
  check('a good solve reports available', r.publishable === true);
  check('acceptance names all three conditions it checks',
    /converged/.test(r.validation.what_it_checks)
    && /every other bus/.test(r.validation.what_it_checks));
  check('the worst bus error is reported, not just the injection bus',
    Number.isFinite(r.validation.worst_bus_error_mw));
  check('the answer names the component it was solved in',
    r.component && Number.isFinite(r.component.buses_in_component));
}

/* CASE 12 - the sink rule is stated, not implicit. */
{
  const g = graphOf(['A', 'B', 'C'], [X('A', 'B', 50), X('B', 'C', 50)]);
  const m = flow.assemble(g, ['A', 'B', 'C'], { voltageKv: 400, includeTransformers: false });
  const sink = flow.sinkFor(m, 'A');
  check('the declared sink is the most connected bus in the same component',
    sink === 'B', String(sink));
  check('an isolated bus has no sink, and that is an answer',
    flow.sinkFor(flow.assemble(graphOf(['Z'], []), ['Z'],
      { voltageKv: 400, includeTransformers: false }), 'Z') === null);
  check('the rule is published so a reader can see it',
    /most connected bus in the SAME component/.test(flow.sink_rule));
}
...
/* THE PRODUCTION PATH ITSELF, on the real product.
   ------------------------------------------------------------------------
   The old proof never ran this. It chose a connected distant slack by hand
   while the live caller took the first lexicographic bus, and that gap is
   the whole reason a broken composition shipped with a green proof. */
console.log('\nthe production caller, on the real network\n');
{
  check('the real 400 kV graph is NOT connected, which is why a slack must be chosen with care',
    model.counts.components > 1, `${model.counts.components} components`);
  const firstBus = model.buses.find(b => b !== model.busOf(wbur));
  check('the bus the OLD caller would have picked is in a different component',
    model.componentOf(firstBus) !== model.componentOf(model.busOf(wbur)),
    `${firstBus}`);
  const wrong = flow.respond(model, { atNode: wbur, slackNode: firstBus, mw: 480, minimumShare: 0 });
  check('and asking for that transfer is refused rather than answered',
    wrong.publishable === false);

  const sink = flow.sinkFor(model, wbur);
  check('the declared sink is in the same component as the injection',
    sink && model.componentOf(sink) === model.componentOf(model.busOf(wbur)), String(sink));
  const real = flow.respond(model, { atNode: wbur, slackNode: sink, mw: 480, minimumShare: 0.05 });
  check('the production path now converges on the real network', real.convergence.converged);
  check('and is available by all three acceptance conditions', real.publishable === true);
  check('every bus in the real solve conserves power to 1e-6 MW',
    real.validation.worst_bus_error_mw < 1e-6,
    `worst ${real.validation.worst_bus_error_mw.toExponential(3)} MW at ${real.validation.worst_bus}`);
  check('it reports flows a reader can act on', real.branches.length > 0);
  check('parallel circuits are counted: the model has more branches than the collapsed version had',
    model.counts.branches > 437, `${model.counts.branches} branches`);
}
