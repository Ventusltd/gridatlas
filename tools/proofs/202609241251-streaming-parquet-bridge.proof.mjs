...
/* Two generations, deliberately, because they are two different things.
   The RUNTIME is restamped every time these bytes change - at 202609241251 it
...
const GENERATION = '202609241251';
const CONTRACT_GENERATION = '202609241251';
...
assert.match(contractSource, /metroPartitionAlias: 'uk_metros_trams_local_unwired'/);
...
const fixtureManifest = {
  ...
  artifacts: [{
    path: 'partitions/uk_metros_trams_local_unwired.parquet',
    sha256: 'a'.repeat(64),
  }],
};
...
/* The seam moved at 202609241251 and this proof moved with it, deliberately.
    ...
    async connect() {
      return {
        query: async (sql) => {
          ...
          return {
            toArray: () => [{
              source_id: 'uk_metros_trams_local_unwired',
              feature_index: 0,
...
assert.match(querySql,
  /read_parquet\('https:\/\/ventusltd\.github\.io\/data-gridatlas\/202608291237-data-gridatlas\/data\/partitions\/uk_metros_trams_local_unwired\.parquet'\)/);
assert.equal(fullState.loaded_on_demand[metroPath].parquet,
  'partitions/uk_metros_trams_local_unwired.parquet');
