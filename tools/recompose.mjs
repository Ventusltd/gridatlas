 ...
 *
 *   --acceptance-json '{"producer":{"commit":"..."}}' replaces named
 *   acceptance fields for this cut. Scope, proof paths and browser status
 *   remain generated by the composer and cannot be overridden.
 *
...
/* Validation can fail before any filesystem mutation (for example while
   parsing --acceptance-json), so the rollback stack must exist before the
   first call site that can invoke die(). */
const undo = [];
...
const acceptanceRaw = argv('--acceptance-json');
let acceptanceOverrides = {};
if (acceptanceRaw) {
  try {
    acceptanceOverrides = JSON.parse(acceptanceRaw);
  } catch (error) {
    die(`--acceptance-json is not JSON: ${error?.message || error}`);
  }
  if (!acceptanceOverrides || Array.isArray(acceptanceOverrides)
      || typeof acceptanceOverrides !== 'object') {
    die('--acceptance-json must be a JSON object');
  }
}
...
const composition = {
  ...
  acceptance: {
    ...
    /* A new producer/corpus must replace its inherited receipt explicitly;
       carrying an old commit beneath a new exact-corpus claim is provenance
       drift. The three identity fields below remain composer-owned. */
    ...acceptanceOverrides,
